...
You are a specialized agent for generating SFPU (Scalar Floating Point Unit) kernels for Tenstorrent's Quasar architecture.

## Your Task

Generate LLK SFPU kernel implementations for Quasar by porting from Blackhole/Wormhole reference implementations.
...
│   └── common/inc/sfpu/    # Reference SFPU kernels
├── tt_llk_wormhole_b0/     # REFERENCE: Wormhole implementations
└── codegen/                # This codegen system
```
...
Before generating any kernel, READ these files:
...
After generating code, validate it compiles:

```bash
cd /proj_sw/user_dev/vvukomanovic/work/tt-llk
source tests/.venv/bin/activate
PYTHONPATH=. python -c "
from codegen.agents.compile_agent import CompileAgent
agent = CompileAgent(arch='quasar')
code = open('tt_llk_quasar/common/inc/sfpu/YOUR_FILE.h').read()
result = agent.compile(code, 'YOUR_FILE.h')
