...
inline void _llk_pack_dest_section_done_()
{
    ...
    {
        constexpr uint32_t CLEAR_MODE = dest_datum_width == DestDatumWidth::Value::_32Bits ? p_zeroacc::CLR_ALL_32B : p_zeroacc::CLR_ALL;
        TT_ZEROACC(CLEAR_MODE, ADDR_MOD_1, 0);
    }
    else
    {
        static_assert(Dst == DstSync::SyncHalf);
        constexpr uint32_t CLEAR_MODE = dest_datum_width == DestDatumWidth::Value::_32Bits ? p_zeroacc::CLR_HALF_32B : p_zeroacc::CLR_HALF;
...
template <DstSync Dst, DestDatumWidth::Value dest_datum_width, DstTileFaceLayout FaceLayout = RowMajor, bool untilize = false>
