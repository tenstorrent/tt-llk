...
inline void eltwise_binary_configure_mop(const uint32_t acc_to_dest = 0, const uint32_t num_faces = 4)
{
    LLK_ASSERT(num_faces == 1 || num_faces == 2 || num_faces == 4, "num_faces must be 1, 2, or 4");
    constexpr bool high_fidelity = is_high_fidelity(math_fidelity);
    const uint32_t addr_mod      = ADDR_MOD_0;
    ...
    if constexpr (bcast_type == BroadcastType::COL || bcast_type == BroadcastType::SCALAR)
    {
        ...
        {
            outerloop = high_fidelity ? to_underlying(math_fidelity) : outerloop;
            ckernel_template tmp(outerloop, innerloop, TT_OP_ELWMUL(0, 0, broadcast_type, addr_mod, 0));
            if constexpr (high_fidelity)
            {
 ...
 *************************************************************************/
inline void eltwise_binary_configure_mop(uint32_t srca_reuse_count = 4)
{
    /*
