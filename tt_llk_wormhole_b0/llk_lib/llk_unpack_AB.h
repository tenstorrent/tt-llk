...
{
    // Move a 16x16 face from L1 to SrcA and SrcB, setting the Dvalid bit for both, telling the MATH it can consume them,
    // static constexpr uint unpack_srca = TT_OP_UNPACR(SrcA, 0b1, 0, 0, 0, 1, 1, p_unpacr::RAREFYB_DISABLE, 0, 0, 0, 0, 1);
    // static constexpr uint unpack_srcb = TT_OP_UNPACR(SrcB, 0b1, 0, 0, 0, 1, 1, p_unpacr::RAREFYB_DISABLE, 0, 0, 0, 0, 1);
    // // Final iteration of the macro-op is the same for now
    ...
    // Commented out for now. The idea is to have three sets of unpack_srca and unpack_srcb and not to set Dvalid bits
    ...
    // Then, the last two instructions after the macro-op inner loop is finished, will set the Dvalid bits.
