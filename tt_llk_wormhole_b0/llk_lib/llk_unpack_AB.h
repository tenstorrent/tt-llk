 ...
 * @tparam BType: Broadcast type for source B, values = <NONE/COL/ROW/SCALAR>
 * @param tensor_shape: Tensor shape describing tile dimensions (face_r_dim, face_c_dim, num_faces_r_dim, num_faces_c_dim)
 * @param transpose_of_faces: Whether to reorder faces (0,2,1,3) (0 = no transpose, >0 = transpose)
 * @param within_face_16x16_transpose: Whether to transpose within each 16x16 face (0 = no transpose, >0 = transpose)
 */
template <BroadcastType BType = BroadcastType::NONE>
