...
inline void _llk_math_eltwise_unary_datacopy_(const std::uint32_t dst_index, const std::uint32_t src_format, const std::uint32_t dst_format)
{
    if (unpack_to_dest && is_32bit_input(src_format, dst_format))
    {
        ...
        math::set_dst_write_addr<DstTileShape::Tile32x32, UnpackToDest::DestReg>(dst_index);
        math::math_unpack_to_dest_tile_ready();
    }
    else
    {
        math::set_dst_write_addr<DstTileShape::Tile32x32, UnpackToDest::SrcRegs>(dst_index);
    ...
    const std::uint32_t dst_index, const std::uint32_t unpack_dst_format, const std::uint32_t unit_dim, const std::uint32_t num_units)
{
    ...
    math::set_dst_write_addr<DstTileShape::Tile32x16, UnpackToDest::SrcRegs>(dst_index);
    ...
    for (uint i = 0; i < num_units; i++)
    {
        ...
        else if (unit_dim == 3)
        {
            ...
            math::set_dst_write_addr<DstTileShape::Tile32x16, UnpackToDest::SrcRegs>(bottom_face_offset);
