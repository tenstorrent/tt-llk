...
template <DestDatumWidth::Value dest_datum_width, bool to_from_int8 = false>
    ...
    const std::uint32_t unpack_src_format, const std::uint32_t unpack_dst_format, const std::uint32_t tile_size)
{
    ...
    {
        static_assert(dest_datum_width, "Reconfiguring unpack to/from Int8 formats requires FP32 Dest mode enabled");
