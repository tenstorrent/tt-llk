...
inline void _llk_math_matmul_(uint dst_index, const std::uint32_t ct_dim = 1, const std::uint32_t rt_dim = 1)
{
    ...
    for (uint t = 0; t < t_dim; t++)
    {
        ...
        {
            math::set_dst_write_addr<DstTileShape::Tile32x32, UnpackToDest::False>(dst_index + (reuse_a ? ct_dim * t + rut : t + rut * ct_dim));
            ...
            else
            {
                ...
                if ((t + 1) < t_dim)
                {
                    ...
                    math::set_dst_write_addr<DstTileShape::Tile32x32, UnpackToDest::False>(
