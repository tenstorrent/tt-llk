...
inline void _calculate_max_pool_with_indices_(const uint32_t values_tile_idx, const uint32_t indices_tile_idx, const uint32_t tile_idx /* unused */)
{
...
inline void _calculate_max_pool_with_indices_generic_(const uint32_t values_tile_idx, const uint32_t indices_tile_idx, const uint32_t tile_idx /* unused */)
{
    // size of each tile in Dest is 64 rows
    constexpr uint32_t dst_tile_size   = 64;
    const uint32_t values_tile_offset  = values_tile_idx * dst_tile_size;
    ...
    auto process_16_rows = [values_tile_offset, indices_tile_offset, eight_row_offset, instr_mod_index](const uint32_t base_offset, const uint32_t col_offset)
                               __attribute__((always_inline))
    {
        // Nested lambda to handle load, sort, and store for a face
        auto load_sort_store = [values_tile_offset, indices_tile_offset, base_offset, col_offset, instr_mod_index](const uint32_t eight_row_offset_val)
                                   __attribute__((always_inline))
        {
            // data
            TT_SFPLOAD(
    ...
    // Final swap
    auto final_swap = [values_tile_offset, indices_tile_offset, sixteen_row_offset, instr_mod_index](const uint32_t col_offset) __attribute__((always_inline))
    {
