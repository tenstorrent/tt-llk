...
inline void _calculate_reshuffle_rows_(const std::uint32_tidx_addr) {
    constexpr std::uint32_toutput_tile_offset = 64;

    // clr DEST tile 1
    // TODO (Radomir): Add optional clear that is more optimal using tile copy
    // for (std::uint32_trow=0; row < 32; row+=4) {
    ...
    for (std::uint32_trow = 0; row < 32; row++) {
        std::uint32_tinput_row_addr = (row < 16) ? ((row / 4) * 4) : ((row / 4) * 4 + 16);
        std::uint32_tinput_row_lreg = input_lreg[row % 4];

        std::uint32_tdst_row = (uint)idx_ptr[row];
        if (dst_row >= 32) { continue; }
        std::uint32_toutput_row_addr = (dst_row < 16) ? ((dst_row / 4) * 4) : ((dst_row / 4) * 4 + 16);
        std::uint32_toutput_row_lreg = output_lreg[dst_row % 4];
