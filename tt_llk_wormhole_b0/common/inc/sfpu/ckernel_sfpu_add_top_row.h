...
inline void _calculate_add_top_row_(const uint32_t tile_idx_0 = 0, const uint32_t tile_idx_1 = 0, const uint32_t tile_idx_dst = 0)
{
    static_assert(
    ...
    constexpr uint32_t REPLAY_BUFFER_INDEX = (format == DataFormat::Float32) ? 4 : 0;
    constexpr uint32_t REPLAY_BUFFER_COUNT = 4;

    // size of each tile in Dest is 64 rows
    constexpr uint32_t dst_tile_size = 64;
    const uint32_t tile_offset_0     = tile_idx_0 * dst_tile_size;
    const uint32_t tile_offset_1     = tile_idx_1 * dst_tile_size;
    const uint32_t tile_offset_dst   = tile_idx_dst * dst_tile_size;
