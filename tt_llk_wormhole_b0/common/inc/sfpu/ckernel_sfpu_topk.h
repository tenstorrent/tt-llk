...
inline void bitonic_topk_ph3_st4_to_1(bool dir, bool &init_replay, int replay_start)
{
    ...
    if (init_replay)
    {
        lltt::record<true>(replay_start, 5);
    ...
    {
        __builtin_rvtt_ttreplay(replay_start, 5, 0, 0);
    }
                ...
                switch (ph)
                {
                    case 0:
                        for (int d = 0; d < 4; d++)
                        {
                            ...
                            if (init_load)
                            {
                                lltt::record<true>(0, 8);
                                bitonic_topk_load16(4, 8);
                                init_load = false;
                            }
                            else
                            {
                                __builtin_rvtt_ttreplay(0, 8, 0, 0);
                            }
                            if (init_phase)
                            {
                                lltt::record<true>(16, 5);
