...
template <ckernel::ApproximationMode APPROX_MODE, int RECIPROCAL_ITERATIONS>
...
#pragma GCC unroll 8
    for (int d = 0; d < iterations; d++)
    {
        ...
        sfpi::vFloat out = _reciprocal_compat_<(APPROX_MODE != ckernel::ApproximationMode::Precise) ? 2 : 3>(in);
        v_if (in < 0.0)
        {
            out = -out;
        }
        v_endif;
        if constexpr (fp32_dest_acc_en || (APPROX_MODE != ckernel::ApproximationMode::Precise))
        {
            sfpi::dst_reg[0] = out;
        }
        else
