...
#pragma GCC unroll 8
    ...
    {
        // operand A - fp32
        TT_SFPLOAD(0, 3, 3, dst_index_in0 * 64);
        // operand B - fp32 scaler
        TT_SFPLOAD(1, 3, 3, dst_index_in1 * 64);
        ...
        TT_SFPSTORE(0, SIGN_MAGNITUDE_FORMAT ? 4 : 12, 3, dst_index_out * 64);
        sfpi::dst_reg++;
    }
}
...
#pragma GCC unroll 8
    ...
    {
        // operand A - int32
        TT_SFPLOAD(0, SIGN_MAGNITUDE_FORMAT ? 4 : 12, 3, dst_index_in0 * 64);
        // operand B - fp32 scaler
        TT_SFPLOAD(1, 3, 3, dst_index_in1 * 64);
        // cast int32->fp32
        TTI_SFPCAST(0, 0, 0);
...
#pragma GCC unroll 8
    for (int d = 0; d < ITERATIONS; d++)
    {
        ...
        TTI_NOP;
        // LREG_0 -> dest as fp32
        TT_SFPSTORE(0, 3, 3, dst_index_out * 64);
        sfpi::dst_reg++;
    }
}
