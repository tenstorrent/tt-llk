...
inline std::uint32_t get_pc_buf_cmd(
    std::uint32_t ckernel_id, std::uint32_t iterations, std::uint32_t number_of_extra_params) {
    // Ckernel ID can be a max of 12 bits.
    return ckernel_id | ((iterations & 0x7FFF) << 16) | ((number_of_extra_params & 0xF) << 12);
}

inline bool is_pc_buf_fast_cmd(std::uint32_t cmd) {
    // MSB stores fast kernel mode
    return (cmd >> 31) & 0x1;
}

inline void decode_pc_buf_cmd(std::uint32_t cmd, std::uint32_t& ckernel_id, std::uint32_t& command_addr) {
    // Ckernel ID can be a max of 12 bits.
...
inline void decode_pc_buf_cmd(
    std::uint32_t cmd, std::uint32_t& ckernel_id, std::uint32_t& iterations, std::uint32_t& number_of_extra_params) {
    // Ckernel ID can be a max of 12 bits.
    ckernel_id             = cmd & 0xFFF;
