...
inline void set_packer_strides(const std::uint32_t pack_src_format, const std::uint32_t pack_dst_format) {
    ...
    std::uint32_t z_stride = PACK_CNT * FACE_C_DIM * y_stride;
    std::uint32_t w_stride = z_stride;
...
inline void set_packer_config(
    ...
    // for (std::uint32_t i=0; i<4; i++) cfg[THCON_SEC0_REG1_Row_start_section_size_ADDR32+i]=config.val[i];
    // for (std::uint32_t i=0; i<4; i++) cfg[THCON_SEC1_REG1_Row_start_section_size_ADDR32+i]=config.val[i];
    // for (std::uint32_t i=0; i<4; i++) cfg[THCON_SEC0_REG8_Row_start_section_size_ADDR32+i]=config.val[i];
    // for (std::uint32_t i=0; i<4; i++) cfg[THCON_SEC1_REG8_Row_start_section_size_ADDR32+i]=config.val[i];
...
inline void set_packer_l1_offset(const std::uint32_t pack_dst_format, const std::uint32_t face_r_dim = FACE_R_DIM) {
    ...
    std::uint32_t l1_offset_1 =
        IS_BFP_FORMAT(pack_dst_format)
            ? 1
            : (((std::uint8_t)(pack_dst_format & 0x3) == (std::uint8_t)DataFormat::Float32)   ? (face_dim / 16) * 4
               : ((std::uint8_t)(pack_dst_format & 0x3) == (std::uint8_t)DataFormat::Float16) ? (face_dim / 16) * 2
                                                                                              : (face_dim / 16));
