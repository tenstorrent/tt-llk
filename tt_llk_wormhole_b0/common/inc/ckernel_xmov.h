...
typedef struct {
    ...
    std::uint32_t no_params : 1; // 0 - use xmov inputs from param registers, 1 - compact move. all inputs are embedded
                                 // into instruction
...
inline void xmov_cfg_instr_set(
    ...
    const std::uint32_t        l1_offset_16B,
    const std::uint32_t        reg_addr32,
    const std::uint32_t        xfer_size = 1) {
...
inline void xmov_cfg_instr_program(
    const risc_compact_mov_instrn_t xmov, const std::uint32_t l1_offset_16B, const std::uint32_t reg_addr32) {
...
inline void xmov_cfg_program(
    const std::uint32_t l1_offset_16B, const std::uint32_t reg_addr32, const std::uint32_t xfer_size = 1) {
    volatile std::uint32_t *XMOV_CMD = reinterpret_cast<volatile std::uint32_t *>(RISCV_TDMA_REG_COMMAND_ADDR);
    // Program tile descriptor using fast XMOV path
    risc_compact_mov_instrn_u risc_mov_instrn;
...
inline void xmov_l1_to_l1_non_compact(
    const std::uint32_t src_addr_16B, const std::uint32_t dst_addr_16B, const std::uint32_t xfer_size_16B = 1) {
    volatile std::uint32_t *XMOV_CMD  = reinterpret_cast<volatile std::uint32_t *>(RISCV_TDMA_REG_COMMAND_ADDR);
