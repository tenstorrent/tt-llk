    ...
    const uint unpB_num_faces       = 4)
{
    ...
    const uint unpA_src_format_masked = static_cast<uint>(unpA_src_format) & 0x0F;
    const uint unpB_src_format_masked = static_cast<uint>(unpB_src_format) & 0x0F;
    const uint unpA_dst_format_masked = static_cast<uint>(unpA_dst_format) & 0x0F;
    const uint unpB_dst_format_masked = static_cast<uint>(unpB_dst_format) & 0x0F;

    // Get pointer to registers for current state ID
    volatile uint tt_reg_ptr *cfg = get_cfg_pointer();

    uint unpA_ch1_x_stride = (unpA_dst_format_masked & 0x3) == to_underlying(DataFormat::Float32)   ? 4
                             : (unpA_dst_format_masked & 0x3) == to_underlying(DataFormat::Float16) ? 2
                                                                                                    : 1;
    uint unpB_ch1_x_stride = (unpB_dst_format_masked & 0x3) == to_underlying(DataFormat::Float32)   ? 4
    ...
    uint exp_width         = (static_cast<uint>(unpA_dst_format_masked) >> 2) & 0x1; // 0=5-bit, 1=8-bit
    ...
    tile_descriptor.f.in_data_format = static_cast<uint>(unpA_src_format_masked);
...
inline void set_dst_write_addr(const uint32_t &context_id, const uint32_t &unpack_dst_format)
{
    ...
    uint unpA_ch1_x_stride = (unpack_dst_format & 0x3) == to_underlying(DataFormat::Float32)   ? 4
