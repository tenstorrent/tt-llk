    ...
    const std::uint32_t unpB_num_faces  = 4)
{
    ...
    const std::uint32_t unpA_src_format_masked = unpA_src_format & 0x0F;
    const std::uint32_t unpB_src_format_masked = unpB_src_format & 0x0F;
    const std::uint32_t unpA_dst_format_masked = unpA_dst_format & 0x0F;
    const std::uint32_t unpB_dst_format_masked = unpB_dst_format & 0x0F;
    ...
    std::uint32_t exp_width         = (unpA_dst_format_masked >> 2) & 0x1; // 0=5-bit, 1=8-bit
    ...
    std::uint32_t int8_math_enabled =
        (unpA_dst_format_masked == to_underlying(DataFormat::Int8)) || (unpB_dst_format_masked == to_underlying(DataFormat::Int8)) ||
