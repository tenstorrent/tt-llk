...
public:
    ...
    static void run(volatile uint *, const uint8_t count, const uint32_t zmask);          // runs - assumes that registers were already programmed
    static void run(volatile uint *, const uint8_t count);                                // runs - assumes that registers were already programmed
...
inline void ckernel_template::program_and_run(volatile uint *)
{
    program(instrn_buffer);
    run(instrn_buffer);
}

inline void ckernel_template::run(volatile uint *)
{
    TTI_MOP(1, 0, 0); // run the double-loop template
}

inline void ckernel_template::program(volatile uint *)
{
...
inline void ckernel_unpack_template::program_and_run(volatile uint *, const uint8_t count, const uint32_t zmask)
{
    program(instrn_buffer);
    run(instrn_buffer, count, zmask);
}

inline void ckernel_unpack_template::run(volatile uint *, const uint8_t count, const uint32_t zmask)
{
...
inline void ckernel_unpack_template::run(volatile uint *, const uint8_t count)
{
    TT_MOP(0, count - 1, 0); // Run the template
}

inline void ckernel_unpack_template::program(volatile uint *) const
{
    volatile uint *mop_cfg = reinterpret_cast<volatile uint *>(TENSIX_MOP_CFG_BASE);
