...
template <DstSync Dst, DstTileShape tile_shape, UnpackDestination unpack_destination>
inline void set_dst_write_addr(uint32_t tile_index)
{
    ...
    constexpr uint dest_size_limit = (Dst == DstSync::SyncHalf) ? DEST_REGISTER_HALF_SIZE : DEST_REGISTER_FULL_SIZE;
