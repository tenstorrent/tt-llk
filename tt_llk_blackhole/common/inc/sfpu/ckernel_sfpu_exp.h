...
inline void _init_exponential_() {
    if constexpr (FAST_APPROX && APPROXIMATION_MODE) {
        ...
        TTI_SFPSHFT(
            15, 0, 15, 1); // imm = 15 to shift left by 15 bits; lreg_c = 0 (will use macro reg); lreg_dest = 15 to
                           // install in Programmable Macro Instruction reg 2'b11, which is Macro Instruction Register 7
        ...
        TTI_SFPLOADI(
            0, 0xA, 0x85DF); // slot1 : SIMPLE UNIT, want SHIFT instruction which is in macro instruction mux[7],
                             ...
                             // 8'b1_______1_______011_____111          = 0xDF slot2 : MAD    UNIT, want MAD instruction
                             // which is in macro instruction mux[5], delayed by 0 ; not using staging flop as dest;
                             // using load reg as srcb : 8'b1_______0_______000_____101          = 0x85
        TTI_SFPLOADI(
