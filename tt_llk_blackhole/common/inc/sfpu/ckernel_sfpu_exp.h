...
namespace exp_fast_approx_detail
{
...
template <int DEST_BASE>
sfpi_inline void loadmacro_compute_block_()
{
    TTI_SFPLOADMACRO(0, 0, ADDR_MOD_7, DEST_BASE + 0); // MACRO Sequence Register 0: LD, MAD, ROUND, SHIFT and STORE - uses LREG[0]
    TTI_SFPLOADMACRO(1, 0, ADDR_MOD_7, DEST_BASE + 2); // MACRO Sequence Register 0: LD, MAD, ROUND, SHIFT and STORE - uses LREG[1]
    TTI_SFPLOADMACRO(2, 0, ADDR_MOD_7, DEST_BASE + 4); // MACRO Sequence Register 0: LD, MAD, ROUND, SHIFT and STORE - uses LREG[2]
    TTI_SFPLOADMACRO(3, 0, ADDR_MOD_7, DEST_BASE + 6); // MACRO Sequence Register 0: LD, MAD, ROUND, SHIFT and STORE - uses LREG[3]
}
} // namespace exp_fast_approx_detail
...
void _calculate_exponential_(const std::uint16_t exp_base_scale_factor /* 1.0f in BF16 */)
{
    if constexpr (FAST_APPROX && APPROXIMATION_MODE && CLAMP_NEGATIVE)
    {
        ...
        {
            TTI_SFPNOP;
            exp_fast_approx_detail::loadmacro_sanitize_block_<8>();
        }

        exp_fast_approx_detail::loadmacro_compute_block_<0>();
        if constexpr (ITERATIONS == 8)
        {
            exp_fast_approx_detail::loadmacro_compute_block_<8>();
        }
