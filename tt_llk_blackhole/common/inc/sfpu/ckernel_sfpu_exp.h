...
template <ckernel::ApproximationMode APPROX_MODE, bool SCALE_EN, bool SKIP_POSITIVE_CHECK>
inline sfpi::vFloat _calculate_exponential_piecewise_(sfpi::vFloat in, const std::uint16_t exp_base_scale_factor /* 1.0f in BF16 */)
{
    static_assert(
        ...
        "Exp piecewise only supports Precise or Approximate modes");
...
template <ckernel::ApproximationMode APPROX_MODE, bool SCALE_EN, int ITERATIONS, bool SKIP_POSITIVE_CHECK>
void _calculate_exponential_(const std::uint16_t exp_base_scale_factor /* 1.0f in BF16 */)
{
    if constexpr (APPROX_MODE == ckernel::ApproximationMode::FastApproximateClamped)
    {
        ...
        (APPROX_MODE == ckernel::ApproximationMode::FastApproximate || APPROX_MODE == ckernel::ApproximationMode::FastApproximateClamped) && ITERATIONS == 8)
    {
        // =======================================================================
        ...
        (APPROX_MODE == ckernel::ApproximationMode::FastApproximate || APPROX_MODE == ckernel::ApproximationMode::FastApproximateClamped) && ITERATIONS == 32)
    {
        // =======================================================================
    ...
    else if constexpr (APPROX_MODE == ckernel::ApproximationMode::FastApproximate || APPROX_MODE == ckernel::ApproximationMode::FastApproximateClamped)
    {
...
template <ckernel::ApproximationMode APPROX_MODE, std::uint32_t scale /* 1.0f in FP32 */>
inline void _init_exponential_()
{
    ...
    else if constexpr (APPROX_MODE == ckernel::ApproximationMode::FastApproximate)
    {
        // ===================================================================
    ...
    else if constexpr (APPROX_MODE != ckernel::ApproximationMode::Precise)
    {
    ...
    {
        // Initialisation for use of _sfpu_reciprocal_<2> in _calculate_exponential_<APPROX_MODE=Precise>.
