...
void _calculate_exponential_(const uint16_t exp_base_scale_factor /* 1.0f in BF16 */)
{
    ...
    else if constexpr (FAST_APPROX && APPROXIMATION_MODE && ITERATIONS == 32)
    {
        ...
        TTI_SFPLOADMACRO(0, 0, ADDR_MOD_7, 0); // LM[0] lreg0, dest 0, Dst -> 2.
        TTI_SFPNOP;
        TTI_SFPLOADMACRO(1, 0, ADDR_MOD_7, 0); // LM[1] lreg1, dest 2, Dst -> 4.
        TTI_SFPNOP;
        ...
        // Manual completion: LM[30], SHFT2[28], LM[31], SHFT2[29].
        TTI_SFPLOADMACRO(2, 0, ADDR_MOD_7, 0);                         // LM[30] lreg2, dest 60, Dst -> 62.
        TTI_SFPSHFT2(p_sfpu::LREG0, p_sfpu::LREG14, p_sfpu::LREG4, 5); // SHFT2[28].
        TTI_SFPLOADMACRO(3, 0, ADDR_MOD_7, 0);                         // LM[31] lreg3, dest 62, Dst -> 64.
...
inline void _init_exponential_()
{
    ...
    else if constexpr (FAST_APPROX && APPROXIMATION_MODE)
    {
        ...
        // Pair 0: LM(LREG2) + SHFT2(LREG0)
        TTI_SFPLOADMACRO(2, 0, ADDR_MOD_7, 0); // lreg2, ADDR_MOD_7, Imm10=0
        TTI_SFPSHFT2(p_sfpu::LREG0, p_sfpu::LREG14, p_sfpu::LREG4, 5);

        // Pair 1: LM(LREG3) + SHFT2(LREG1)
        TTI_SFPLOADMACRO(3, 0, ADDR_MOD_7, 0); // lreg3, ADDR_MOD_7, Imm10=0
        TTI_SFPSHFT2(p_sfpu::LREG1, p_sfpu::LREG14, p_sfpu::LREG4, 5);

        // Pair 2: LM(LREG0) + SHFT2(LREG2)
        TTI_SFPLOADMACRO(0, 0, ADDR_MOD_7, 0); // lreg0, ADDR_MOD_7, Imm10=0
        TTI_SFPSHFT2(p_sfpu::LREG2, p_sfpu::LREG14, p_sfpu::LREG4, 5);

        // Pair 3: LM(LREG1) + SHFT2(LREG3)
        TTI_SFPLOADMACRO(1, 0, ADDR_MOD_7, 0); // lreg1, ADDR_MOD_7, Imm10=0
