...
/**
 * @file ckernel_sfpu_topk.h
 * @brief Top-K selection algorithm implementation for SFPU hardware
 *
 * @details This file implements the Top-K selection algorithm using SFPU hardware
 * acceleration, providing efficient selection of the K largest (or smallest) elements
 * from input tensors. Top-K is a fundamental operation in machine learning,
 * particularly for attention mechanisms, beam search, and ranking applications.
 *
 * **Top-K Algorithm Overview:**
 * - **Selection**: Find the K largest/smallest elements from input data
 * - **Sorting**: Optionally sort the selected K elements
 * - **Index Tracking**: Maintain original indices of selected elements
 * - **Partial Sorting**: Avoid full sorting for better performance
 *
 * **Implementation Strategies:**
 * 1. **Heap-based Selection**: Min/max heap for K-element selection
 * 2. **Quickselect Variant**: Partition-based selection for large K
 * 3. **Parallel Processing**: SIMD-optimized comparison and selection
 * 4. **Memory Efficient**: In-place operations where possible
 *
 * **SFPU Optimization Features:**
 * - **Vector Comparisons**: Parallel comparison across 32 SIMD lanes
 * - **Conditional Execution**: Hardware-accelerated conditional logic
 * - **Load/Store Efficiency**: Optimized memory access patterns
 * - **Configuration Integration**: Uses SFPU configuration and macro loading
 *
 * **Key Algorithm Components:**
 * - **Partition Logic**: Efficient data partitioning for selection
 * - **Comparison Networks**: SIMD-optimized comparison operations
 * - **Index Management**: Tracking original element positions
 * - **Boundary Handling**: Proper handling of edge cases and ties
 *
 * **Input/Output Format:**
 * - **Input**: Tensor data with configurable dimensions and layout
 * - **Values Output**: Selected K largest/smallest values
 * - **Indices Output**: Original positions of selected elements
 * - **Sorting**: Optional sorting of selected elements
 *
 * **Template Configuration:**
 * - **K Value**: Number of elements to select (compile-time or runtime)
 * - **Direction**: Largest vs. smallest selection
 * - **Sorting**: Whether to sort selected elements
 * - **Index Tracking**: Whether to maintain original indices
 *
 * **Performance Characteristics:**
 * - **Complexity**: O(n log k) for heap-based, O(n) average for quickselect
 * - **SIMD Efficiency**: 32-way parallel processing where applicable
 * - **Memory Usage**: Optimized for cache efficiency and minimal allocation
 * - **Scalability**: Efficient scaling with input size and K value
 *
 * **Hardware Integration:**
 * - **SFPU Instructions**: Leverages comparison, conditional, and data movement
 * - **Configuration**: Uses SFPU configuration registers for algorithm tuning
 * - **Memory Access**: Optimized load/store patterns for tensor data
 * - **Pipeline Efficiency**: Minimizes pipeline stalls and maximizes throughput
 *
 * **Common Use Cases:**
 * - Attention mechanism top-K selection in transformers
 * - Beam search in sequence generation models
 * - Sparse matrix operations and nearest neighbor search
 * - Ranking and recommendation system kernels
 * - Computer vision top-K pooling operations
 */
