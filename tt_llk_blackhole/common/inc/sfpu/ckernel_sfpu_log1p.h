...
sfpi_inline sfpi::vFloat _calculate_log1p_bf16_(sfpi::vFloat val)
{
    sfpi::vFloat abs_x = sfpi::abs(val);
    sfpi::vFloat result;
    v_if (abs_x < 0.0078125)
    {                 // use 2^(-7) as threshold value
        result = val; // log(1+x) ~ x for x < 0.01
    }
    v_else
    {
        sfpi::vFloat in = val + sfpi::vConst1;
        result          = _calculate_log_body_no_init_(in);
        // result = _calculate_log_body_vec_<FAST_APPROX, /*HAS_BASE_SCALING*/ false, /*is_fp32_dest_acc_en*/ true>(in, 0);
    }
    v_endif;
...
sfpi_inline sfpi::vFloat _calculate_log1p_fp32_(sfpi::vFloat val)
{
    ...
    v_else
    {
        ...
        v_else
        {
            ...
            // where z = (m - 1) / (m + 1)
            sfpi::vFloat p =
                PolynomialEvaluator::eval(z2, sfpi::vConst1, 0.3333333333333333f, 0.2f, 0.14285714285714285f, 0.1111111111111111f, .09090909090909091f);
...
#pragma GCC unroll 8
    for (int d = 0; d < ITERATIONS; d++)
    {
        ...
        if constexpr (is_fp32_dest_acc_en)
        {
            result = _calculate_log1p_fp32_<is_fp32_dest_acc_en>(in);
        }
        else
        {
            result = _calculate_log1p_bf16_<FAST_APPROX, is_fp32_dest_acc_en>(in);
        }
