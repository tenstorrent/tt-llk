 ...
 *        For INT32 MIN operations, includes format conversion from sign+magnitude to 2's complement.
 *
 * @tparam INSTRUCTION_MODE The instruction mode for integer and float formats: INT32, INT32_2S_COMP, LO16, FP32, FP16B
 * @tparam pool_type The operation type: MAX or MIN
...
inline void init_reduce_max(uint32_t num_cols)
{
    ...
    // Bit 8 (0x0100) controls swap direction for min/max comparisons
    // For INT32 sign+magnitude, we also need format conversion to 2's complement
    ...
    // For INT32 sign+magnitude MIN: convert to 2's complement, compare, convert back
    constexpr bool use_format_conversion = (INSTRUCTION_MODE == InstrModLoadStore::INT32 && pool_type == PoolType::MIN);
    constexpr uint32_t replay_length     = use_format_conversion ? 19 : 11;

    lltt::record<lltt::NoExec>(0, replay_length);
    TTI_INCRWC(0, 4, 0, 0);
    ...
    if constexpr (use_format_conversion)
    {
        // Convert LREG0,1 from sign+magnitude to 2's complement
        TTI_SFPCAST(p_sfpu::LREG0, p_sfpu::LREG2, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG0, 0); // Blackhole RTL workaround
        TTI_SFPCAST(p_sfpu::LREG1, p_sfpu::LREG3, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG3, p_sfpu::LREG1, 0); // Blackhole RTL workaround

        // Convert LREG6,7 from sign+magnitude to 2's complement
        TTI_SFPCAST(p_sfpu::LREG6, p_sfpu::LREG2, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG6, 0);
        TTI_SFPCAST(p_sfpu::LREG7, p_sfpu::LREG3, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG3, p_sfpu::LREG7, 0);

        // Perform swap on 2's complement values
        ...
        // Convert results back to sign+magnitude
        TTI_SFPCAST(p_sfpu::LREG0, p_sfpu::LREG2, InstrModCast::INT32_2S_COMP_TO_INT_SIGN_MAGN);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG0, 0);
        TTI_SFPCAST(p_sfpu::LREG1, p_sfpu::LREG3, InstrModCast::INT32_2S_COMP_TO_INT_SIGN_MAGN);
        TTI_SFPSETSGN(0, p_sfpu::LREG3, p_sfpu::LREG1, 0);
    }
    else
    {
        // Standard swap without format conversion
        TTI_SFPSWAP(0 /*unused*/, p_sfpu::LREG7 /*lreg_src_c*/, p_sfpu::LREG1 /*lreg_dest*/, 1 /*instr_mod1*/);
        TTI_SFPNOP;
        TTI_SFPSWAP(0 /*unused*/, p_sfpu::LREG6 /*lreg_src_c*/, p_sfpu::LREG0 /*lreg_dest*/, 1 /*instr_mod1*/);
        TTI_SFPNOP;
 ...
 *        for each of the columns across all rows in the block. The result values are placed into
 ...
 *        - For INT32 MIN: Converts from sign+magnitude to 2's complement format before comparisons
 ...
 *        - For INT32 MIN: Converts results back from 2's complement to sign+magnitude format
...
inline void calculate_reduce_max(const uint32_t block_height)
{
    ...
    // For INT32 MIN: Convert initial values from sign+magnitude to 2's complement
    if constexpr (use_format_conversion)
    {
        TTI_SFPCAST(p_sfpu::LREG4, p_sfpu::LREG2, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG4, 0);
        TTI_SFPCAST(p_sfpu::LREG5, p_sfpu::LREG2, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG5, 0);
        TTI_SFPCAST(p_sfpu::LREG6, p_sfpu::LREG2, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG6, 0);
        TTI_SFPCAST(p_sfpu::LREG7, p_sfpu::LREG2, InstrModCast::INT_SIGN_MAGN_TO_INT32_2S_COMP);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG7, 0);
    }
    ...
    // For INT32 MIN: Convert results back from 2's complement to sign+magnitude before storing
    if constexpr (use_format_conversion)
    {
        TTI_SFPCAST(p_sfpu::LREG4, p_sfpu::LREG2, InstrModCast::INT32_2S_COMP_TO_INT_SIGN_MAGN);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG4, 0);
        TTI_SFPCAST(p_sfpu::LREG5, p_sfpu::LREG2, InstrModCast::INT32_2S_COMP_TO_INT_SIGN_MAGN);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG5, 0);
        TTI_SFPCAST(p_sfpu::LREG6, p_sfpu::LREG2, InstrModCast::INT32_2S_COMP_TO_INT_SIGN_MAGN);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG6, 0);
        TTI_SFPCAST(p_sfpu::LREG7, p_sfpu::LREG2, InstrModCast::INT32_2S_COMP_TO_INT_SIGN_MAGN);
        TTI_SFPSETSGN(0, p_sfpu::LREG2, p_sfpu::LREG7, 0);
    }
