...
#define TT_OP_SEMINIT(max_value, init_value, sem_sel) \
  TT_OP(0xa3, (((max_value) << 20) + ((init_value) << 16) + ((sem_sel) << 0)))
...
#define TT_OP_SEMWAIT(stall_res, sem_bank_sel, sem_sel, wait_sem_cond) \
  TT_OP(0xa6, (((stall_res) << 15) + ((wait_sem_cond) << 13) + ((sem_bank_sel) << 8) + ((sem_sel) << 0)))
#define TT_SEMWAIT_VALID(stall_res, sem_bank_sel, sem_sel, wait_sem_cond) \
  (ckernel::is_valid(stall_res, 9) && ckernel::is_valid(sem_bank_sel, 1) && ckernel::is_valid(sem_sel, 8) && \
        ckernel::is_valid(wait_sem_cond, 2))
#define TT_SEMWAIT(stall_res, sem_bank_sel, sem_sel, wait_sem_cond) \
  ckernel::instrn_buffer[0] = TT_OP_SEMWAIT(stall_res, sem_bank_sel, sem_sel, wait_sem_cond)
// BH backward compatibility means that sem_bank_sel is always 0
#define TTI_SEMWAIT(stall_res, sem_sel, wait_sem_cond) \
   INSTRUCTION_WORD(TRISC_OP_SWIZZLE(TT_OP_SEMWAIT(stall_res, 0, sem_sel, wait_sem_cond)))
