...
inline void _llk_math_transpose_dest_(const uint32_t dst_index)
{
...
#pragma GCC unroll 2
        ...
        uint32_t macro0 = TT_OP_SFPNOP;
        uint32_t macro1 = TT_OP_SFPNOP;
        ...
        uint32_t movd2b_hi        = lltt::replay_insn(16, 4);
        uint32_t movb2d_hi_d2b_lo = lltt::replay_insn(20, 8);
        uint32_t movb2d_lo        = lltt::replay_insn(28, 4);
        uint32_t transpose        = TT_OP_TRNSPSRCB;
    ...
    else
    {
        ...
        uint32_t EFGHIJKLM   = lltt::replay_insn(20, 9);
        uint32_t EFGHI       = lltt::replay_insn(20, 5);
        uint32_t ABCDEFG     = lltt::replay_insn(16, 7);
        uint32_t P           = TT_OP_MOVD2B(0, 28, ADDR_MOD_2, p_movd2b::MOV_4_ROWS, 12); // dst -= 16
        uint32_t IJKL        = lltt::replay_insn(24, 4);
        uint32_t Q           = TT_OP_MOVB2D(0, 28, ADDR_MOD_3, p_movb2d::MOV_4_ROWS, 12); // dst += 32
        uint32_t EFGHIJKLMNO = lltt::replay_insn(20, 11);
