...
inline void _llk_pack_dest_section_done_()
{
    ...
    {
        TT_ZEROACC(p_zeroacc::CLR_ALL, dest_datum_width == DestDatumWidth::Value::_32Bits, 0, ADDR_MOD_1, 0);
    }
    else
    {
        static_assert(Dst == DstSync::SyncHalf);
        TT_ZEROACC(p_zeroacc::CLR_HALF, dest_datum_width == DestDatumWidth::Value::_32Bits, 0, ADDR_MOD_1, (dest_offset_id) % 2);
    }
...
template <DstSync Dst, DestDatumWidth::Value dest_datum_width, DstTileFaceLayout FaceLayout = RowMajor>
