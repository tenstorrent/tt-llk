    ...
    const bool partial_face            = false)
{
    ...
    const std::uint32_t replay_buf_len = is_in0_16x32 ? (partial_face ? 4 : 8) : 16;

    load_replay_buf(
        ckernel::math::replay_buf_offset,
        replay_buf_len,
        // Lambda function to load reply buffer
        [high_fidelity, partial_face, is_in0_16x32]
        {
            if (is_in0_16x32)
            {
                if (partial_face)
            ...
            {
                // reuse_a = false
                TTI_MVMUL(p_setrwc::CLR_B, 0, ADDR_MOD_5, 0); // B3A3 or B2A1 // reset srca/srcb/dest, increment phase (addr_mod_5), clear src B
            }
        });
...
{
    craqmm_configure_addrmod<MATH_FIDELITY_DESC, FaceLayout>(
        transpose, kt_dim, in0_tile_r_dim, in0_tile_c_dim, in1_tile_r_dim, in1_tile_c_dim, partial_face);

    constexpr int MATH_FIDELITY_PHASES = get_math_num_fidelity_phases(MATH_FIDELITY_DESC);
    craqmm_configure_mop<MATH_FIDELITY_PHASES, FaceLayout>(
