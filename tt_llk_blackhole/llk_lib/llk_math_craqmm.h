...
{
    craqmm_configure_addrmod<MATH_FIDELITY_DESC, FaceLayout>(transpose, kt_dim, in0_tile_r_dim, in0_tile_c_dim, in1_tile_r_dim, in1_tile_c_dim, partial_face);

    constexpr int MATH_FIDELITY_PHASES = get_math_num_fidelity_phases(MATH_FIDELITY_DESC);
    craqmm_configure_mop<MATH_FIDELITY_PHASES, FaceLayout>(transpose > 0, kt_dim, in0_tile_r_dim, in0_tile_c_dim, in1_tile_r_dim, in1_tile_c_dim, partial_face);

    math::reset_counters(p_setrwc::SET_ABD_F);
}

// Simplified implementation: NUM_FIDELITY_PHASES = 0 (no high fidelity mode)
template <DstTileFaceLayout FaceLayout = DstTileFaceLayout::ColMajor>
inline void _llk_math_craqmm_(uint dst_index, [[maybe_unused]] const bool transpose = false, [[maybe_unused]] const std::uint32_t kt_dim = 1)
{
