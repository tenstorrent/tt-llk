...
public:
    ...
    void program(volatile uint *);                    // just programs the registers
    void program_bank0_sw_cntl(volatile uint *);      // programs BANK0 in software control mode
    void program_bank1_sw_cntl(volatile uint *);      // programs BANK1 in software control mode
    static void run(volatile uint *);                 // runs - assumes that registers were already programmed
    static void run_and_finish(volatile uint *);      // runs and switches mop_config bank - assumes that registers were already programmed
    static void run_bank0_sw_cntl(volatile uint *);   // run bank 0 in SW control mode
    static void run_bank1_sw_cntl(volatile uint *);   // run bank 1 in SW control mode
    void program_and_run(volatile uint *);            // calls program, then run
    void program_and_run_and_finish(volatile uint *); // calls program, then runs and switches the mop_config bank
};
...
public:
    ...
    */
    ...
    void program(volatile uint *) const;         // just programs the registers
    static void run(volatile uint *, const uint8_t count, const uint32_t zmask = 0);      // runs - assumes that registers were already programmed
    void program_and_run(volatile uint *, const uint8_t count, const uint32_t zmask = 0); // calls program, then run
...
void ckernel_template::program_and_run(volatile uint *)
{
    program(instrn_buffer);
    run(instrn_buffer);
}

void ckernel_template::program_and_run_and_finish(volatile uint *)
{
    program(instrn_buffer);
    run_and_finish(instrn_buffer);
}

void ckernel_template::run(volatile uint *)
{
    TTI_MOP(1, 0, 0, 0); // run the double-loop template
}

void ckernel_template::run_bank0_sw_cntl(volatile uint *)
{
    TTI_MOP(1, 0, 0, 0); // run the double-loop template
}

void ckernel_template::run_and_finish(volatile uint *)
{
    TTI_MOP(1, 1, 0, 0); // run the double-loop template
}

void ckernel_template::run_bank1_sw_cntl(volatile uint *)
{
    TTI_MOP(1, 1, 0, 0); // run the double-loop template
}

void ckernel_template::program(volatile uint *)
{
...
void ckernel_template::program_bank0_sw_cntl(volatile uint *)
{
    volatile mop_config_regs_t *mop_cfg = reinterpret_cast<volatile mop_config_regs_t *>(MOP_CFG_BASE);
...
void ckernel_template::program_bank1_sw_cntl(volatile uint *)
{
    volatile mop_config_regs_t *mop_cfg = reinterpret_cast<volatile mop_config_regs_t *>(MOP_CFG_BASE);
...
#if 0
void ckernel_unpack_template::program_and_run(volatile uint *, const uint8_t count, const uint32_t zmask)
{
  program(instrn_buffer);
  run(instrn_buffer, count, zmask);
}

void ckernel_unpack_template::run(volatile uint *, const uint8_t count, const uint32_t zmask)
{
...
void ckernel_unpack_template::program(volatile uint *) const
{
