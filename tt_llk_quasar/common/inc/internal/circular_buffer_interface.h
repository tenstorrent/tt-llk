...
//     std::uint32_t* cb_l1_base,
//     std::uint32_t start_cb_index,
//     std::uint32_t max_cb_index,
//     bool read,
//     bool write) {
//     volatile std::uint32_t* circular_buffer_config_addr =
//         cb_l1_base + start_cb_index * UINT32_WORDS_PER_LOCAL_CIRCULAR_BUFFER_CONFIG;

//     for (std::uint32_t cb_id = start_cb_index; cb_id < max_cb_index; cb_id++) {
//         // NOTE: fifo_addr, fifo_size and fifo_limit in 16B words!
//         std::uint32_t fifo_addr = circular_buffer_config_addr[0] >> CIRCULAR_BUFFER_COMPUTE_ADDR_SHIFT;
//         std::uint32_t fifo_size = circular_buffer_config_addr[1] >> CIRCULAR_BUFFER_COMPUTE_ADDR_SHIFT;
//         std::uint32_t fifo_num_pages = circular_buffer_config_addr[2];
//         std::uint32_t fifo_page_size = circular_buffer_config_addr[3] >> CIRCULAR_BUFFER_COMPUTE_ADDR_SHIFT;
