...
inline void _llk_unpack_binary_broadcast_operands_mop_config_(const uint32_t buf_desc_id_0, const uint32_t buf_desc_id_1, const uint32_t num_tiles)
{
    ...
    uint32_t unpack_srca_tile_inc            = TT_OP_UNPACR0_TILE_INC(0, 1 /*Src Tile Idx*/, buf_desc_id_0, 1 /*Set Dvalid*/);
    constexpr static uint32_t replay_buf_len = (BROADCAST_TYPE == BroadcastType::SCALAR) ? 1 : 4;
 ...
 */
inline void _llk_unpack_binary_broadcast_operands_(const uint32_t start_l1_tile_idx_0, const uint32_t start_l1_tile_idx_1)
{
