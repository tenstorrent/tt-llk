...
inline void _llk_math_eltwise_unary_broadcast_mop_config_(const TileShape& tile_shape)
{
    ...
    const std::uint32_t num_rows_per_matrix = (BROADCAST_TYPE == BroadcastType::SCALAR) ? tile_shape.num_faces * tile_shape.face_r_dim : tile_shape.face_r_dim;
...
inline void _llk_math_eltwise_unary_broadcast_d2b_mop_config_(const TileShape& tile_shape)
{
    ...
    else // COL
    {
        ...
        const std::uint32_t num_rows_to_transfer = tile_shape.face_r_dim * 2; // 2 faces
