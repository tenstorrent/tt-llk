...
void run_kernel(const volatile struct RuntimeParams* params)
{
    ...
    for (int block = 0; block < num_blocks; block++)
    {
        ...
        {
            int tile_idx = (block * num_tiles_in_block / params->SRCA_REUSE_COUNT) + tile;
            _llk_unpack_bcastA_B_(L1_ADDRESS(buffer_A[tile_idx]), L1_ADDRESS(buffer_B[tile_idx * params->SRCA_REUSE_COUNT]), params->SRCA_REUSE_COUNT);
        }
    }
}
...
void run_kernel(const volatile struct RuntimeParams* params)
{
    ...
    for (int block = 0; block < num_blocks; block++)
    {
        ...
        for (int tile = 0; tile < num_tiles_in_block / params->SRCA_REUSE_COUNT; tile++)
        {
            int dst_index = (block * num_tiles_in_block) + (tile * params->SRCA_REUSE_COUNT);
...
void run_kernel(const volatile struct RuntimeParams* params)
{
    ...
    for (int block = 0; block < num_blocks; block++)
    {
        ...
        for (int tile = 0; tile < num_tiles_in_block; tile++)
        {
            ...
            _llk_pack_<dest_sync, is_fp32_dest_acc_en, false>(res_tile_idx, L1_ADDRESS(buffer_Res[res_tile_idx]));
        }
