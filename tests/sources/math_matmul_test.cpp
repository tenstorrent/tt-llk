...
void run_kernel(const volatile struct RuntimeParams *params)
{
    ...
    LLK_ASSERT(
        (get_max_dst_index_for_matmul(params->DST_INDEX, params->CT_DIM, params->RT_DIM) <
...
void run_kernel(const volatile struct RuntimeParams *params)
{
    ...
    {
        LLK_ASSERT((params->DST_INDEX + i < get_dest_max_tiles<dest_sync, is_fp32_dest_acc_en, DstTileShape::Tile32x32>()), "i exceeds max dest tiles");
