...
{
#ifdef ARCH_BLACKHOLE
    _llk_pack_hw_configure_<dest_datum_width, false, false>(formats.pack_src, formats.pack_dst, 16 * 16);
#else
    _llk_pack_hw_configure_<dest_datum_width, false>(formats.pack_src, formats.pack_dst, 16 * 16);
#endif
    ...
    for (int i = 0; i < TILE_CNT; i++)
    {
        ...
        _llk_pack_<DstSync::SyncHalf, dest_datum_width, false>(0, L1_ADDRESS(buffer_Res[i]));
