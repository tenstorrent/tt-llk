 ...
 *
 * This measures the performance improvement of using all 4 packer interfaces
 * compared to the standard 2-interface implementation.
 *
 * Expected speedup: ~2x for even block_ct_dim, ~1.8x for odd block_ct_dim
 */
...
void run_kernel(RUNTIME_PARAMETERS params)
{
...

    {
        ...
        _llk_pack_untilize_4intf_init_<BLOCK_CT_DIM, FULL_CT_DIM>(formats.pack_src, formats.pack_dst, FACE_R_DIM, 4);
#else
...

    {
        ...
        if constexpr (PERF_RUN_TYPE == PerfRunType::PACK_ISOLATE || PERF_RUN_TYPE == PerfRunType::L1_CONGESTION)
        {
            for (std::uint32_t loop = 0; loop < LOOP_FACTOR; loop++)
            {
                for (std::uint32_t tile = 0; tile < TILE_CNT; tile += BLOCK_CT_DIM)
                {
                    ...
                    _llk_pack_untilize_4intf_<BLOCK_CT_DIM, FULL_CT_DIM>(PERF_ADDRESS(PERF_OUTPUT, tile), formats.pack_dst, FACE_R_DIM, 4, 0);
                }
            }
            PROFILER_SYNC();
            return;
        ...
        for (std::uint32_t loop = 0; loop < LOOP_FACTOR; loop++)
        {
            for (std::uint32_t i = 0; i < TILE_CNT; i += BLOCK_CT_DIM)
            {
                ...
                // Use 4-interface optimized pack API
                _llk_pack_untilize_4intf_<BLOCK_CT_DIM, FULL_CT_DIM>(PERF_ADDRESS(PERF_OUTPUT, i), formats.pack_dst, FACE_R_DIM, 4, 0);
