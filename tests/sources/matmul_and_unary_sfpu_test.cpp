...
void run_kernel()
{
    ...
    _llk_unpack_reconfig_data_format_srca_impl_<is_fp32_dest_acc_en>(formats_array[run].unpack_src, formats_array[run].unpack_dst);
...
void run_kernel()
{
    int run = 0; // first L1-to-L1 run, we access the first set of formats_array in our array
#ifdef ARCH_BLACKHOLE
    _llk_pack_hw_configure_<is_fp32_dest_acc_en>(formats_array[run].pack_src, formats_array[run].pack_dst, FACE_R_DIM, TILE_C_DIM, num_faces);
    _llk_pack_init_<false, false, false>(formats_array[run].pack_dst);
#else
    _llk_pack_hw_configure_<is_fp32_dest_acc_en>(formats_array[run].pack_src, formats_array[run].pack_dst, FACE_R_DIM, num_faces);
    ...
    run = 1; // second L1-to-L1 run, we access the second set of formats_array in our array
    _llk_pack_reconfig_data_format_<is_fp32_dest_acc_en>(formats_array[run].pack_src, formats_array[run].pack_dst);
