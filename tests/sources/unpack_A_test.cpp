...
void run_kernel(const volatile struct RuntimeParams *params)
{
    ...
    for (int block = 0; block < num_blocks; block++)
    {
        ...
        {
            _llk_math_eltwise_unary_datacopy_<copy_type, sync_mode, is_fp32_dest_acc_en, BROADCAST_TYPE, unpack_to_dest>(tile, formats.math, formats.math);
        }
...
void run_kernel(const volatile struct RuntimeParams *params)
{
    ...
    for (int block = 0; block < num_blocks; block++)
    {
        ...
        for (int tile = 0; tile < num_tiles_in_block; tile++)
        {
            ...
            _llk_pack_<sync_mode, is_fp32_dest_acc_en, false>(tile, L1_ADDRESS(buffer_Res[res_tile_idx]));
        }
