...
void run_kernel()
{
    ...
    _llk_unpack_AB_matmul_init_<>(UNPACK_TRANSPOSE_FACES, CT_DIM, RT_DIM, KT_DIM, FACE_R_DIM, FACE_R_DIM, num_faces, num_faces);
...
void run_kernel()
{
    _llk_math_matmul_init_<MATH_FIDELITY, DstTileFaceLayout::RowMajor>(
        TILE_R_DIM, TILE_C_DIM, TILE_R_DIM, TILE_C_DIM, PARTIAL_FACE, UNPACK_TRANSPOSE_FACES, CT_DIM, RT_DIM, KT_DIM);
...
void run_kernel()
{
    ...
    _llk_pack_hw_configure_<is_fp32_dest_acc_en, false, false>(formats.pack_src, formats.pack_dst, TILE_SIZE_PACK, FACE_R_DIM, TILE_C_DIM, num_faces);
...
#else
    _llk_pack_hw_configure_<is_fp32_dest_acc_en, false>(formats.pack_src, formats.pack_dst, TILE_SIZE_PACK, FACE_R_DIM, num_faces);
