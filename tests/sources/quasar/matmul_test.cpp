...
void run_kernel()
{
    ...
    tdma_desc_src_a.buf_desc.f.x_dim        = FACE_R_DIM;  // Default face dimension is 16, tiny tiles not supported for quasar
    ...
    tdma_desc_src_b.buf_desc.f.x_dim        = FACE_R_DIM;  // Default face dimension is 16, tiny tiles not supported for quasar
    ...
    _llk_unpack_matmul_init_<29, 30, UNPACK_TRANSPOSE_FACES, CT_DIM, RT_DIM, KT_DIM>(); // transpose in src_A not supported for quasar

    for (uint32_t j = 0; j < KT_DIM; j++)
    {
        _llk_unpack_matmul_<29, 30, CT_DIM, RT_DIM, KT_DIM>(j, j * CT_DIM);
    }
}
