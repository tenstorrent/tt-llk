...
void run_kernel(const volatile struct RuntimeParams *params)
{
    ...
    _llk_math_srcAB_hw_configure_<
        ...
        static_cast<DataFormat>(formats.math)>();
    _llk_math_matmul_init_<(ckernel::MathFidelity)MATH_FIDELITY, false, false>(params->CT_DIM, params->RT_DIM); // disable flags for matmul with indexing and
                                                                                                                // mxfp_2x not part of P0 test suite

    for (uint32_t i = 0; i < params->KT_DIM; i++)
    {
...
void run_kernel(const volatile struct RuntimeParams *params)
{
    ...
    _llk_pack_matmul_init_<p_pacr::PACK0>(buf_desc_id_dst, params->RT_DIM, params->CT_DIM, 1); // Use destination buffer descriptor for packing output
