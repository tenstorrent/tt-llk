...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    // Dispatch based on SFPU_UNARY_OPERATION
    constexpr SfpuType op = SFPU_UNARY_OPERATION;

    // Init for inverse hyperbolic ops (no-op on Quasar, but maintains API symmetry)
    if constexpr (op == SfpuType::acosh || op == SfpuType::asinh)
    {
        ckernel::sfpu::_init_inverse_hyperbolic_();
    }
    else if constexpr (op == SfpuType::atanh)
    {
        ckernel::sfpu::_init_atanh_();
    }

    // Apply SFPU trig operation to all tiles
    for (std::uint32_t i = 0; i < params.TILE_CNT; ++i)
    {
        if constexpr (op == SfpuType::sine)
        {
            _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_sine_<false>, i, num_sfpu_iterations);
        }
        else if constexpr (op == SfpuType::cosine)
        {
            _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_cosine_<false>, i, num_sfpu_iterations);
        }
        ...
        {
            _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_atanh_, i, num_sfpu_iterations);
        }
    }
