...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    // Initialize hardsigmoid constants (slope=1/6, offset=0.5, upper_clamp=1.0)
    _init_hardsigmoid_();

    // Apply SFPU hardsigmoid to all tiles
    for (std::uint32_t i = 0; i < params.TILE_CNT; ++i)
    {
        _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_hardsigmoid_, i, num_sfpu_iterations);
    }
