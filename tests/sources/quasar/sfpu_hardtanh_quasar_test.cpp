...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    // Hardtanh parameters: min_val=-1.0f, max_val=1.0f (PyTorch defaults)
    // IEEE 754 float32: -1.0f = 0xBF800000, 1.0f = 0x3F800000
    ...
    // Apply SFPU hardtanh to all tiles
    for (std::uint32_t i = 0; i < params.TILE_CNT; ++i)
    {
        _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_hardtanh_, i, num_sfpu_iterations, min_val_param, max_val_param);
    }
