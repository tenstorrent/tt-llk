...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    // The kernel extracts (slope >> 16) to get the BF16 immediate for SFPLOADI
    constexpr std::uint32_t alpha_param = 0x3F800000u; // 1.0f

    // Apply SFPU ELU to all tiles
    for (std::uint32_t i = 0; i < params.TILE_CNT; ++i)
    {
        _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_elu_, i, num_sfpu_iterations, alpha_param);
    }
