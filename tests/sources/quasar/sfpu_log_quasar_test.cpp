...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    // Initialize log kernel (loads polynomial coefficients into LREGs)
    _init_log_();

    // Apply SFPU log to all tiles
    for (std::uint32_t i = 0; i < params.TILE_CNT; ++i)
    {
        _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_log_, i, num_sfpu_iterations);
    }
