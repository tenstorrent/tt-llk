...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    // CELU parameters:
    // alpha = 1.0f -> IEEE 754 float32 = 0x3F800000
    // alpha_recip = 1/alpha = 1.0f -> IEEE 754 float32 = 0x3F800000
    // The kernel extracts (param >> 16) to get the BF16 immediate for SFPLOADI
    constexpr std::uint32_t alpha_param = 0x3F800000u;       // 1.0f
    constexpr std::uint32_t alpha_recip_param = 0x3F800000u; // 1.0f (1/1.0)

    // Apply SFPU CELU to all tiles
    for (std::uint32_t i = 0; i < params.TILE_CNT; ++i)
    {
        _llk_math_eltwise_unary_sfpu_params_<false>(ckernel::sfpu::_calculate_celu_, i, num_sfpu_iterations, alpha_param, alpha_recip_param);
    }
