...
void run_kernel()
{
...

    {
        ...
        {
            _llk_unpack_A_<>(L1_ADDRESS(src_a + (tile % 8) * 0x1000), UNPACK_TRANSPOSE_FACES, formats.unpack_src, formats.unpack_dst); // TODO LP HERE
        }
        ckernel::tensix_sync(); // -> perf
    }
}
...
void run_kernel()
{
    ...
    }
    {
        ...
        for (uint32_t tile = 0; tile < TILE_CNT; tile++)
        {
            ...
            _llk_pack_<DstSync::SyncHalf, is_fp32_dest_acc_en>(0, L1_ADDRESS(dst + (tile % 8) * 0x1000)); // TODO LP HERE
