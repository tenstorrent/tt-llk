...
constexpr std::uint32_t SUB_RT_DIM    = 2;
constexpr std::uint32_t SUB_OUT_TILES = SUB_CT_DIM * SUB_RT_DIM;
...
constexpr std::uint32_t SUB_A_ADDR      = 0x1A000;
constexpr std::uint32_t SUB_B_ADDR      = 0x1E000;
constexpr std::uint32_t SUB_OUT0_ADDR   = 0x1F000;
constexpr std::uint32_t MATMUL_A_ADDR   = 0x23000;
constexpr std::uint32_t MATMUL_B_ADDR   = 0x27000;
constexpr std::uint32_t MATMUL_OUT_ADDR = 0x2F000;
constexpr std::uint32_t SUB_OUT1_ADDR   = 0x33000;
...
{
    const Operand sub_a(SUB_A_ADDR, SUB_OUT_TILES * TILE_SIZE_BYTES);
    const Operand sub_b(SUB_B_ADDR, SUB_RT_DIM * TILE_SIZE_BYTES);
    ...
    for (std::uint32_t row = 0; row < SUB_RT_DIM; ++row)
    {
        _llk_unpack_AB_sub_bcast_col_custom_<BroadcastType::COL>(L1_ADDRESS(sub_a[row * SUB_CT_DIM]), L1_ADDRESS(sub_b[row]), SUB_CT_DIM);
    }
...
void run_kernel(RUNTIME_PARAMETERS params)
{
    ...
    for (std::uint32_t row = 0; row < SUB_RT_DIM; ++row)
    {
        math::set_dst_write_addr<DstTileShape::Tile32x32, UnpackDestination::SrcRegs>(row * SUB_CT_DIM);
...
void run_kernel(RUNTIME_PARAMETERS params)
{
    const Operand sub_out0(SUB_OUT0_ADDR, SUB_OUT_TILES * TILE_SIZE_BYTES);
    const Operand matmul_out(MATMUL_OUT_ADDR, MATMUL_OUT_TILES * TILE_SIZE_BYTES);
    const Operand sub_out1(SUB_OUT1_ADDR, SUB_OUT_TILES * TILE_SIZE_BYTES);
    const std::uint32_t format = ckernel::to_underlying(DataFormat::Float16_b);
    ...
    {
        _llk_pack_<DstSync::SyncFull, false, false>(tile, L1_ADDRESS(sub_out0[tile]));
    }
    ...
    {
        _llk_pack_<DstSync::SyncFull, false, false>(tile, L1_ADDRESS(matmul_out[tile]));
    }
    _llk_pack_dest_section_done_<DstSync::SyncFull, false>();
    t6_semaphore_post<>(semaphore::PACK_DONE);
    ...
    for (std::uint32_t tile = 0; tile < SUB_OUT_TILES; ++tile)
    {
        _llk_pack_<DstSync::SyncFull, false, false>(tile, L1_ADDRESS(sub_out1[tile]));
    }
    _llk_pack_dest_section_done_<DstSync::SyncFull, false>();
}
