...
#include "llk_math_matmul_custom.h"
#include "params.h"

void run_kernel(const volatile struct RuntimeParams *params)
{
    _llk_math_matmul_custom_init_<MATH_FIDELITY>(0, params->CT_DIM, params->RT_DIM);

    matmul_custom_preload_replay_buf_<MATH_FIDELITY>(params->CT_DIM, params->RT_DIM);
    ...
    for (uint32_t j = 0; j < params->KT_DIM; j++)
    {
        _llk_math_matmul_custom_<MATH_FIDELITY>(0, params->CT_DIM, params->RT_DIM);
    }
