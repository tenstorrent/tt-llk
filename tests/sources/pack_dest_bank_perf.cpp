...
{
    {
        ...
        _llk_pack_dest_bank_init_<false, false>(params->TILE_CNT, FACE_R_DIM, params->num_faces);
        ...
        if constexpr (PERF_RUN_TYPE == PerfRunType::PACK_ISOLATE || PERF_RUN_TYPE == PerfRunType::L1_CONGESTION)
        {
            for (int loop = 0; loop < LOOP_FACTOR; ++loop)
            {
                for (uint32_t tile = 0; tile < params->TILE_CNT; tile++)
                {
                    ...
                    _llk_pack_dest_bank_<DstSync::SyncHalf, is_fp32_dest_acc_en>(tile, PERF_ADDRESS(PERF_OUTPUT, tile), 1);
                }
            }
        }
        else
