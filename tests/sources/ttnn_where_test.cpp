...
void run_kernel(const volatile struct RuntimeParams*)
{
    ...
    ckernel::sfpu::_init_where_<ckernel::ApproximationMode::Precise>();

    _llk_math_eltwise_ternary_sfpu_start_<DstSync::SyncHalf>(0);

    constexpr int iterations = 32;

    ckernel::sfpu::_calculate_where_<ckernel::ApproximationMode::Precise, static_cast<DataFormat>(UNPACK_A_IN), iterations>(0, 1, 2, 0);
