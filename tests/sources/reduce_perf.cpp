...
void run_kernel()
{
    ...
    constexpr bool IS_INT_FPU       = false;
    {
        ...
        _llk_math_reduce_init_<POOL_TYPE, REDUCE_DIM, dest_datum_width, MATH_FIDELITY, ENFORCE_FP32_ACC>(/* transpose within face */ IS_REDUCE_ROW);
        PROFILER_SYNC();
    }
    {
        ZONE_SCOPED("TILE_LOOP")
        ...
        else if constexpr (PERF_RUN_TYPE == PerfRunType::MATH_ISOLATE)
        {
            for (uint32_t block_start = 0; block_start < TILE_CNT; block_start += MAX_TILES_DEST)
            {
                ...
                {
                    _llk_math_reduce_<POOL_TYPE, REDUCE_DIM, dest_datum_width, MATH_FIDELITY, IS_INT_FPU, ENFORCE_FP32_ACC>(block_tile);
                }
            }
        }
        else
