...
inline void device_setup()
{
    ...
    constexpr std::uint32_t TRISC_CONFIG_REGS[] = {
        TRISC_RESET_PC_SEC0_PC_ADDR32,
        TRISC_RESET_PC_SEC1_PC_ADDR32,
        TRISC_RESET_PC_SEC2_PC_ADDR32
    };

    volatile std::uint32_t* const trisc_start_addresses = 
        reinterpret_cast<volatile std::uint32_t*>(TRISC_START_BASE);
    volatile uint tt_reg_ptr* cfg_regs = 
        reinterpret_cast<volatile uint tt_reg_ptr*>(TENSIX_CFG_BASE);
