...
mailbox_t brisc_bread2 = mailboxes_arr + 8;
...
{
    counter++;
    commit_store(brisc_command_buffer + (counter & 1), static_cast<std::uint32_t>(BriscCommandState::IDLE_STATE));
...
int main()
{
    ...
    ckernel::store_blocking(brisc_bread2, 0);
    ...
    while (true)
    {
        ...
        switch (static_cast<BriscCommandState>(ckernel::load_blocking(brisc_command_buffer + (counter & 1))))
        {
            ...
            case BriscCommandState::RESET_TRISCS:
                ...
                break;
            case BriscCommandState::DO_EBREAK:
                reset_state(counter);
                commit_store(brisc_bread2, counter);
                asm volatile("ebreak; nop; nop; nop; nop; nop; nop; nop; nop;");
                break;
            default:
                break;
        }
