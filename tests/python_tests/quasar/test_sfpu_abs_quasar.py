...
def prepare_abs_inputs(
    ...
    """
    Prepare input tensor for abs operation with good value coverage.

    Abs does not expand or contract value ranges (|x| has the same magnitude as x),
    so there is no overflow risk. We use a log-uniform distribution to get coverage
    across orders of magnitude, with both positive and negative values.
    ...
    Returns:
        Prepared tensor with mixed positive and negative values
    """
    input_torch_format = format_dict[input_format]
    input_finfo = torch.finfo(input_torch_format)

    # Abs is safe for all finite values — no overflow risk
...
# Format list from planner spec: codegen/artifacts/abs_phase1_spec.md
...
def test_sfpu_abs_quasar(formats_dest_acc_implied_math_input_dims):
    ...
    # Prepare inputs with good value coverage for abs operation
