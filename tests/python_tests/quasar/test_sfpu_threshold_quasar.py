...
# Threshold constants matching the C++ test source.
# threshold=5.0 and value=10.0 match the golden generator defaults (t=5, v=10).
THRESHOLD_CONST = 5.0
VALUE_CONST = 10.0


def prepare_threshold_inputs(
    src_A: torch.Tensor,
    src_B: torch.Tensor,
    ...
    """
    Prepare input tensor for threshold operation with values spanning both
    sides of the threshold (5.0).

    Values below the threshold get replaced with the value constant (10.0).
    Values above the threshold pass through unchanged.

    Uses a log-uniform distribution centered around the threshold to ensure
    good test coverage of both branches.
    ...
    Returns:
        Prepared tensor with values spanning both sides of threshold
    """
    ...
    # Output must fit in the output format. For threshold, the maximum output
    # is max(input, VALUE_CONST=10.0), so the input itself must fit in the
    ...
    # Create values that span both sides of the threshold:
    # ~50% below threshold (will be replaced with VALUE_CONST)
    # ~50% above threshold (will pass through unchanged)
    # Range: [-max_safe_value, max_safe_value] with good coverage near threshold
    result = torch.zeros_like(src_A_float)

    # Below threshold: range from -max_safe_value to just below threshold
    below_mask = src_B_normalized < 0.5
    below_values = -max_safe_value + src_A_normalized * (THRESHOLD_CONST - 0.1 + max_safe_value)

    # Above threshold: range from just above threshold to max_safe_value
    above_values = (THRESHOLD_CONST + 0.1) + src_A_normalized * (max_safe_value - THRESHOLD_CONST - 0.1)

    result = torch.where(below_mask, below_values, above_values)
...
def generate_sfpu_threshold_combinations(
    formats_list: List[FormatConfig],
):
    """
    Generate SFPU threshold test combinations.
...
# Float-only SFPU op per planner spec (codegen/artifacts/threshold_phase1_spec.md
# "Recommended Test Formats"): threshold operates on FP32 in SFPU, constants
# are loaded as BF16. Integer formats excluded.
SFPU_THRESHOLD_FORMATS = input_output_formats(
    [
        DataFormat.Float16,
...
@pytest.mark.quasar
@parametrize(
    formats_dest_acc_implied_math_input_dims=generate_sfpu_threshold_combinations(
        SFPU_THRESHOLD_FORMATS
    ),
)
def test_sfpu_threshold_quasar(formats_dest_acc_implied_math_input_dims):
    """
    Test threshold operation on Quasar architecture.

    Threshold replaces input values <= 5.0 with 10.0, and passes through values
    > 5.0 unchanged. Uses PyTorch's torch.nn.functional.threshold(input, 5.0, 10.0)
    ...
    # Prepare inputs with values spanning both sides of the threshold
    src_A = prepare_threshold_inputs(
    ...
    configuration = TestConfig(
        "sources/quasar/sfpu_threshold_quasar_test.cpp",
        formats,
        templates=[
            MATH_OP(mathop=MathOperation.Threshold),
