...
def prepare_sign_inputs(
    ...
    """
    Prepare input tensor for sign operation with good value coverage.

    Sign maps all values to {-1.0, 0.0, +1.0}, so there is no overflow risk.
    We use a log-uniform distribution for magnitudes with random signs, plus
    explicit zeros to exercise all three code paths (positive, negative, zero).
    ...
    Returns:
        Prepared tensor with positive, negative, and zero values
    """
    input_torch_format = format_dict[input_format]
    input_finfo = torch.finfo(input_torch_format)

    # Sign is safe for all finite values -- no overflow risk
    ...
    # Inject some exact zeros to exercise the zero code path
    # Set approximately 5% of values to zero
    num_elements = src_A_values.numel()
    num_zeros = max(1, num_elements // 20)
    zero_indices = torch.randperm(num_elements)[:num_zeros]
    src_A_values.view(-1)[zero_indices] = 0.0
...
def generate_sfpu_sign_combinations(
    formats_list: List[FormatConfig],
):
    """
    Generate SFPU sign test combinations.
...
# Format list from planner spec: codegen/artifacts/sign_phase1_spec.md
...
@pytest.mark.quasar
@parametrize(
    formats_dest_acc_implied_math_input_dims=generate_sfpu_sign_combinations(
        SFPU_SIGN_FORMATS
    ),
)
def test_sfpu_sign_quasar(formats_dest_acc_implied_math_input_dims):
    """
    Test sign operation on Quasar architecture.

    Uses PyTorch's sign as the golden reference and generates input stimuli
    covering the full representable range with positive, negative, and zero values.
    """
    ...
    # Prepare inputs with good value coverage for sign operation
    src_A = prepare_sign_inputs(
    ...
    configuration = TestConfig(
        "sources/quasar/sfpu_sign_quasar_test.cpp",
        formats,
        templates=[
            MATH_OP(mathop=MathOperation.Sign),
