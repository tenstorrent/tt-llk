...
# ELU alpha constant matching the C++ test source.
# alpha=1.0 matches the golden generator default (torch.nn.functional.elu with alpha=1.0).
ELU_ALPHA = 1.0
...
def prepare_elu_inputs(
    ...
    """
    Prepare input tensor for ELU operation.

    ELU(x) = x if x >= 0, alpha * (exp(x) - 1) if x < 0.
    For negative inputs, exp(x) must not underflow. The SFPU hardware exp
    approximation works well for moderate negative values. Limit input range
    to avoid extreme values that cause overflow/underflow.
    ...
    # For ELU, the positive path is identity so positive values must fit in output.
    ...
    # exp(x) underflows to 0 and ELU saturates at -alpha.
...
# Float-only SFPU op per planner spec (codegen/artifacts/elu_phase1_spec.md
# "Recommended Test Formats"): ELU requires exp() which is float-only.
...
@pytest.mark.quasar
@parametrize(
    formats_dest_acc_implied_math_input_dims=generate_sfpu_elu_combinations(
        SFPU_ELU_FORMATS
    ),
)
def test_sfpu_elu_quasar(formats_dest_acc_implied_math_input_dims):
    """
    Test ELU activation on Quasar architecture.

    ELU(x) = x if x >= 0, alpha * (exp(x) - 1) if x < 0.
    Uses alpha=1.0 matching the C++ test constant and golden generator default.
    Uses PyTorch's torch.nn.functional.elu(input, alpha=1.0) as the golden reference.
    """
