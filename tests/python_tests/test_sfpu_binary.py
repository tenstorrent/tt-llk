...
def test_sfpu_binary_add_top_row(test_name, formats, dest_acc, mathop):
    ...
    golden_tensor = generate_golden(
        ...
        1,  # num_iterations: 1 iteration for top row
        input_dimensions,
        formats.output_format,
    )[
        0:32
    ]  # Only first 32 elements (top row)
...
def sfpu_binary(test_name, formats, dest_acc, mathop):
    ...
    generate_golden = get_golden_generator(BinarySFPUGolden)
    # src_A already contains 2 tiles (for input_dimensions=[32, 64])
    # Use src_A as both source 0 and source 1 (they are separate tiles within src_A)
    golden_tensor = generate_golden(
        mathop,
        src_A,  # Contains tiles 0 and 1
        0,  # src1_idx: use tile 0
        ...
        input_dimensions,  # [32, 64] = 2 tiles
        (
            DataFormat.Float16_b
