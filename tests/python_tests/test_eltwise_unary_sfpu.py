...
all_format_combos = generate_format_combinations(["Float16_b", "Float16", "Float32"], True)
all_params = generate_params(["eltwise_unary_sfpu_test"], all_format_combos, 
                             dest_acc= ["", "DEST_ACC"], 
...
def test_eltwise_unary_sfpu(testname, formats, dest_acc, approx_mode, mathop):#
    ...
    res_from_L1 = collect_results(formats, sfpu=True) # Bug patchup in (unpack.py): passing formats struct to check unpack_src with pack_dst and distinguish when input and output formats have different exponent widths then reading from L1 changes
