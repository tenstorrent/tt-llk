...
# Must match the C++ constants in stream_h2d_test.cpp
...
def test_stream_h2d(workers_tensix_coordinates):
    ...
    # Zero stream memory before starting kernels
    write_to_device(location, STREAM_ADDRESS, bytes(8 + STREAM_DEPTH))

    elfs = config.run_elf_files(location)

    # Host → device: push deterministic packets while MATH kernel pops
    stream = Stream(STREAM_ADDRESS, STREAM_DEPTH, location)
    rng = Prng(DATA_SEED)
    for packet_size in PACKET_SIZES:
        packet = bytes(rng.byte() for _ in range(packet_size))
        stream.push(packet)
