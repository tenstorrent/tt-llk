...
def generate_random_face(
    ...
    elif stimuli_format in [DataFormat.MxFp6R, DataFormat.MxFp6P]:
        # MXFP6 optimized stimuli generation
        return _generate_mxfp6_face(stimuli_format, size, const_face, const_value, sfpu)
...
def _generate_mxfp6_face(stimuli_format, size, const_face, const_value, sfpu):
    """
    Generate test data for MXFP6 formats using normal distribution scaled to format range.
    ...
    if stimuli_format == DataFormat.MxFp6R:
        scale = 0.05 * MX_FORMAT_MAX_NORMAL[DataFormat.MxFp6R]
    else:  # MxFp6P
        scale = 0.05 * MX_FORMAT_MAX_NORMAL[DataFormat.MxFp6P]
...
def _clamp_mx_tensors(
    ...
    if (
        ...
        # Then MxFp6P (±7.5), MxFp6R (±28.0), MxFp8P (±448.0), or MxFp8R (±57344.0) in order of restrictiveness
        # Check for most restrictive format present in the pair
        for fmt in [
            DataFormat.MxFp4,
            DataFormat.MxFp6P,
            DataFormat.MxFp6R,
            ...
            if fmt in [stimuli_format_A, stimuli_format_B]:
                clamp_format = fmt
                break
        else:
