...
"""
    ...
    _llk_unpack_AB_matmul_hw_configure_<is_fp32_dest_acc_en, StochRndType::None>({UNPACK_A_IN}, {UNPACK_A_IN}, {UNPACK_A_OUT}, {UNPACK_A_OUT}, {FACE_R_DIM}, {FACE_R_DIM}, 0, 4, 4, {TILE_SIZE_UNPACK_A}, {TILE_SIZE_UNPACK_B});
    _llk_unpack_AB_matmul_init_<>(0, {CT_DIM}, {RT_DIM}, {KT_DIM}, {FACE_R_DIM}, {FACE_R_DIM});
    for (uint32_t j = 0; j < {KT_DIM}; j++)
    {{
        _llk_unpack_AB_matmul_<>(L1_ADDRESS(buffer_A{stage}[0]), L1_ADDRESS(buffer_B{stage}[0]), j, j * {CT_DIM}, {TILE_SIZE_UNPACK_A}, {TILE_SIZE_UNPACK_B}, {FACE_R_DIM}, {FACE_R_DIM}, false, false, {CT_DIM}, {RT_DIM}, {KT_DIM});
    }}
"""
        return code
