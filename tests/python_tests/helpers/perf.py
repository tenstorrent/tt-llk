...
def perf_benchmark(
    ...
    # Combine all results - each DataFrame has different columns (stats)
    # pd.concat will automatically create all columns from all DataFrames, filling missing values with NaN
    results_combined = pd.concat(results, ignore_index=True)

    # Group by marker and take the first non-null value for each column
    report = results_combined.groupby("marker").first().reset_index()
