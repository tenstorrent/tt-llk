...
class ScalarBroadcastGolden:
    ...
    Output size = num_faces * (face_r_dim * face_c_dim) elements, all with the same scalar value.
    """
...
class ColumnBroadcastGolden:
    ...
    For a face_r_dim x face_c_dim face: input has face_r_dim unique values (one per row),
    each value is replicated face_c_dim times across its row.
    Output pattern: [row0_val]*face_c_dim, [row1_val]*face_c_dim, ..., [row(face_r_dim-1)_val]*face_c_dim
    """
    ...
    def __call__(
        ...
        # Each face is face_r_dim x face_c_dim elements
...
class DataCopyGolden:
    def __call__(
        ...
        tile_size = face_r_dim * face_c_dim * num_faces
        tile_cnt = (height * width) // tile_size

        # Calculate elements based on variable face dimensions
        # Each face is face_r_dim × face_c_dim, and we have num_faces
        elements_per_tile_needed = tile_size
