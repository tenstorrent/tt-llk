...
class TopKGolden:
    """
    Golden generator for simple LLK-style TopK over flat tile payloads.

    Semantics:
      - Operand is a flat tensor containing `tile_cnt` concatenated tiles.
      - Each tile is treated as a 1D sequence of length `N` (N <= ELEMENTS_PER_TILE).
      - For each tile:
          * Take the first N elements as the logical sequence.
          * Run torch.topk over that sequence.
          * Write the top-k values into the first k positions of the tile.
          * Leave the rest of the tile unchanged (or optionally zeroed).

    This is intentionally simple and LLK-focused; it does NOT try to reproduce
    the full TTNN TopK layout (no 4D reshape, no separate indices tensor).
    """
    ...
    def __call__(
        ...
        """
        Args:
            operand: 1D tensor or list with tile_cnt * ELEMENTS_PER_TILE elements.
            data_format: LLK DataFormat (e.g. Float16_b).
            k: number of top elements per tile.
            N: logical sequence length per tile (N <= ELEMENTS_PER_TILE).
            tile_cnt: number of tiles concatenated in operand.
            largest: whether to select largest (True) or smallest (False) k elements.
            sorted: whether the k elements should be sorted.
            zero_tail: if True, zero out elements [k..N-1] in each tile; otherwise left unchanged.

        Returns:
            Tensor of same shape/dtype as operand with top-k values placed
            at the front of each tile.
        """
        if k <= 0:
            raise ValueError(f"k must be positive, got {k}")
        if N <= 0 or N > ELEMENTS_PER_TILE:
            raise ValueError(f"N must be in (0, {ELEMENTS_PER_TILE}], got {N}")
        if k > N:
        ...
        if isinstance(operand, torch.Tensor):
            flat = operand.flatten().to(torch_format).clone()
        else:
            flat = torch.tensor(operand, dtype=torch_format).flatten()

        expected_elements = tile_cnt * ELEMENTS_PER_TILE
        if flat.numel() != expected_elements:
            raise ValueError(
                f"Operand size mismatch: got {flat.numel()} elements, "
                f"expected {expected_elements} ({tile_cnt} tiles × {ELEMENTS_PER_TILE})"
            )
        ...
        for tile_idx in range(tile_cnt):
            ...
            # Logical sequence is first N elements of this tile
            seq = tile[:N].to(torch.float32)
            ...
            # Write top-k values at the front of the tile
            tile[:k] = values.to(torch_format)

            # Optionally zero the remainder of the logical sequence
            if zero_tail and N > k:
                tile[k:N] = torch.zeros(N - k, dtype=torch_format)

            # Write back
            result[tile_start:tile_end] = tile
