...
from . import (
    device as device_module,  # We do this in order to change 'Mailbox' enum values based on coverage options
)
...
from .llk_params import DestAccumulation, L1Accumulation, MailboxDebug, MailboxNonDebug
...
class TestConfig:
    ...
    DATA_FORMAT_ENUM_VALUES: ClassVar[dict]
    ...
    def setup_arch():
        ...
        match TestConfig.CHIP_ARCH:
            case ChipArchitecture.WORMHOLE:
                ...
                TestConfig.DATA_FORMAT_ENUM_VALUES = {
    ...
    def setup_compilation_options(
        ...
        device_module.Mailbox = MailboxDebug if with_coverage else MailboxNonDebug
    ...
    def generate_runtime_args_struct(self):
        ...
        if not self.compile_time_formats and self.formats:
            ...
            if self.L1_to_L1_iterations == 1:
                lines.append("struct FormatConfig formats;")
                self.runtime_format += "IIIII"  # 5 uint32_ts constituting format struct
            else:
                lines.append(
                    f"struct FormatConfig formats[{self.L1_to_L1_iterations}];"
                )
                self.runtime_format += (
                    self.L1_to_L1_iterations * "IIIII"
    ...
    def generate_compile_time_formats(self):
            ...
            for formats_config in self.formats_config:
                argument_data.extend(
                    [
                        ...
                        TestConfig.DATA_FORMAT_ENUM_VALUES[formats_config.unpack_A_dst],
                        TestConfig.DATA_FORMAT_ENUM_VALUES[formats_config.math],
                        TestConfig.DATA_FORMAT_ENUM_VALUES[formats_config.pack_src],
                        TestConfig.DATA_FORMAT_ENUM_VALUES[formats_config.pack_dst],
                    ]
                )
    ...
    def run_with_retries(self, location="0,0", with_stimuli: bool = True):
        retry_count = 0
        while True:
            ...
            try:
                ...
                retry_count += 1
                if retry_count == TestConfig.MAX_RETRY_COUNT:
                    raise Exception(
                        f"WTF handler happened {TestConfig.MAX_RETRY_COUNT} times in a row!!! | Mailboxes (little-endian): {dump_mailboxes(location).hex()}"
                    )
    ...
    def run(self, location="0,0"):
        ...
        self.run_with_retries(location)
