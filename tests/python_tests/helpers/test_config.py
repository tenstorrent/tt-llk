...
class TestConfig:
    ...
    def generate_variant_hash(self):
        ...
        # To account for subtle changes in addresses that are result of stimuli dimensions change
        temp_str.extend(
            [
                str(self.variant_stimuli.buf_a_addr),
                str(self.variant_stimuli.buf_b_addr),
                str(
                    self.variant_stimuli.buf_c_addr
                    if hasattr(self.variant_stimuli, "buf_c_addr")
                    else ""
                ),
            ]
        )
