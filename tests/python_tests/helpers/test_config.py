...
class TestConfig:
    ...
    def run_elf_files(self, location="0,0") -> list:
        ...
        temp_boot_mode = self.boot_mode
        if temp_boot_mode == BootMode.DEFAULT:
            temp_boot_mode = CHIP_DEFAULT_BOOT_MODES[TestConfig.CHIP_ARCH]

        if (
            TestConfig.CHIP_ARCH == ChipArchitecture.QUASAR
            and temp_boot_mode != BootMode.TRISC
        ):
