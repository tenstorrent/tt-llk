...
def run_elf_files(testname, core_loc="0,0", run_brisc=True):
    ...
    a = read_word_from_device(core_loc, RISC_DBG_SOFT_RESET0)
    a = a | 0x7800
    write_words_to_device(core_loc, RISC_DBG_SOFT_RESET0, a)
# self.RISC_DBG_SOFT_RESET0 = device.get_tensix_register_address("RISCV_DEBUG_REG_SOFT_RESET_0")
# a = read_word_from_device(core_loc, self.RISC_DBG_SOFT_RESET0)
    for i in range(3):
...
def assert_value_with_timeout(core_loc, mailbox_addr, timeout=0, poll_interval=0.1):
    ...
    assert (
        read_word_from_device(core_loc, mailbox_addr) == 2
