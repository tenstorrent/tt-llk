...
def pass_fail_results(formats, dest_acc: Optional[DestAccumulation] = None, approx_mode: Optional[ApproximationMode] = None, mathop: Optional[MathOperation]=None, math_fidelity: Optional[MathFidelity]=None, tile_cnt: Optional[TileCount]=None, reduce_dim: Optional[ReduceDimension]=None, pool_type: Optional[ReducePool]=None):
    hw = "PASS"
    res = [
        "FAIL",  # Result
        ...
        (formats.math.name),
        (formats. pack_src.name, formats.pack_dst.name),  # Output Formats
    ]
...
def format_included_params(results, pass_pass, pass_fail, fail_pass, fail_fail):
    for x in results[9:]:
...
def format_results(results):
    ...
    headers_pass = ["RESULT", "HW", "INPUT FORMAT", "OUTPUT FORMAT", "PCC", "Unpack Src", "Unpack Dst", "FPU", "Pack Src", "Pack Dst"]
    ...
    headers_fail_fail = ["RESULT", "HW", "INPUT FORMAT", "OUTPUT FORMAT", "ERROR", "Unpack Src", "Unpack Dst", "FPU", "Pack Src", "Pack Dst"]
