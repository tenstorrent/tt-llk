...
def sweep_integers(int_formats: List[DataFormat]):
    formats = []
    for unpack_in in range(len(int_formats)):
        for unpack_out in range(len(int_formats)):
            for math in range(len(int_formats)):
                for pack_in in range(len(int_formats)):
                    for pack_out in range(len(int_formats)):
                        formats.append(
                            FormatConfig(
                                unpack_A_src=int_formats[unpack_in],
                                unpack_A_dst=int_formats[unpack_out],
                                unpack_B_src=int_formats[unpack_in],  # Assuming same src format for simplicity
                                unpack_B_dst=int_formats[unpack_out],  # Assuming same dst format for simplicity
                                pack_src=int_formats[pack_in],
                                pack_dst=int_formats[pack_out],
                                math=int_formats[math],
                            )
                        )
    return formats


def pass_fail_results(
    testname,
    formats,
    dest_acc: Optional[DestAccumulation] = None,
    approx_mode: Optional[ApproximationMode] = None,
    mathop: Optional[MathOperation] = None,
    math_fidelity: Optional[MathFidelity] = None,
    tile_cnt: Optional[int] = None,
    reduce_dim: Optional[ReduceDimension] = None,
    pool_type: Optional[ReducePool] = None,
):
    res = [
        "FAIL",  # Result
...
def format_results(results):
    ...
    for entry in results:
        ...
        #     fail_pass_results.append(entry[0:4] + entry[5:])
            ...
            fail_fail_results.append(
                [entry[0]] + entry[2:4] + entry[6:]
            )  # (entry[0:4] + entry[5:])
    ...
    with open("test_results.txt", "w") as file:
        ...
        file.write(
            tabulate(
                sorted_fail_fail_results, headers=headers_fail_fail, tablefmt="grid"
            )
        )
