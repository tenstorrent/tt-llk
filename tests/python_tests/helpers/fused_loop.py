...
class LoopRowByRow(FusedLoop):
    ...
    def math_loop(
        ...
        code = ""
        for tile_y in range(block_size_y):
            code += compute_unit.fpu.calculate(operation, config, compute_unit, tile_y)

        return code


class LoopColumnByColumn(FusedLoop):
    def unpack_loop(
        self,
        operation: FusedOperation,
        config: GlobalConfig,
    ...
    def math_loop(
        ...
        code = ""
        for tile_x in range(block_size_x):
            code += compute_unit.fpu.calculate(operation, config, compute_unit, tile_x)
