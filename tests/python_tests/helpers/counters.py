...
def measure_perf_counters(
    run_test_func,
    test_config,
    boot_mode,
    unpack_counters: List[Dict] = None,
    math_counters: List[Dict] = None,
    pack_counters: List[Dict] = None,
    location: str = "0,0",
    mode: str = "GRANTS",
) -> Dict[str, List[Dict]]:
    """
    Unified performance counter measurement with automatic C++ detection or explicit Python configuration.

    If no counter lists provided (all None), auto-detects counters from C++ code.
    Otherwise, uses the provided Python counter lists.

    Args:
        run_test_func: Function to run the test
        test_config: Test configuration
        boot_mode: Boot mode
        unpack_counters: Optional list of UNPACK counters (Python mode)
        math_counters: Optional list of MATH counters (Python mode)
        pack_counters: Optional list of PACK counters (Python mode)
        location: Device location
        mode: Counter mode ("GRANTS" or "REQUESTS")

    Returns:
        Dictionary with keys 'UNPACK', 'MATH', 'PACK' containing counter results
    """

    # Determine mode: C++ auto-detection or Python explicit configuration
    cpp_mode = (
        unpack_counters is None and math_counters is None and pack_counters is None
    )

    if cpp_mode:
        # C++ MODE: Auto-detect counters from C++ code
        print(f"\n{'='*80}")
        ...
        # PYTHON MODE: Use provided counter lists
        unpack_counters = unpack_counters or []
        math_counters = math_counters or []
        pack_counters = pack_counters or []
    ...
    # Common batching logic for both C++ and Python modes
    ...
    for batch_idx in range(max_batches):
        ...
        # Configure threads with batch-specific counters
        # Always configure all three threads to clear any old configs
        configure_perf_counters(
            batch_unpack if batch_unpack else [], location, "UNPACK", mode
        )
        configure_perf_counters(
            batch_math if batch_math else [], location, "MATH", mode
        )
        configure_perf_counters(
            batch_pack if batch_pack else [], location, "PACK", mode
        )
...
def measure_perf_counters_from_cpp(
    ...
    """Legacy wrapper - use measure_perf_counters() instead."""
    return measure_perf_counters(
        run_test_func, test_config, boot_mode, location=location
    )
...
def measure_perf_counters_indexed(
    ...
    return measure_perf_counters(
        ...
        test_config,
        boot_mode,
        unpack_counters=unpack_counters,
        math_counters=math_counters,
