...
class Packer:
    ...
    def pack(self, operation_config: "FusedOperation") -> str:
        ...
        code = (
            ...
            f"    const uint32_t pack_src_format{stage} = ckernel::to_underlying(DataFormat::{pack_src.name});\n"
            f"    const uint32_t pack_dst_format{stage} = ckernel::to_underlying(DataFormat::{pack_dst.name});\n"
        )
