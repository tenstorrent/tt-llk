...
from .format_arg_mapping import DataCopyType, DestAccumulation
...
def generate_params(
    ...
    data_copy_type: Optional[DataCopyType] = None,
) -> List[tuple]:
    """
    ...
    return [
        ...
        for data_copy in (data_copy_type if data_copy_type is not None else [None])
    ]
...
def generate_param_ids(included_params, all_params: List[tuple]) -> List[str]:
    ...
    def format_combination(comb: tuple) -> str:
        ...
        if params[7]:
            result.append(f"data_copy_type={params[7].name}")
