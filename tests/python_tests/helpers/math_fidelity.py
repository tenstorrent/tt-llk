...
def apply_fidelity(operand1, operand2, data_format, math_fidelity_phase):
    ...
    if data_format == DataFormat.Float16:
        ...
        # print('\n'.join([' '.join([f"{x.item():04x}" for x in row]) for row in operand1_uint.view(32, 32)]))
        # print("Operand2 (bfloat16) as hex:")
        # print('\n'.join([' '.join([f"{x.item():04x}" for x in row]) for row in operand2_uint.view(32, 32)]))
    ...
    if data_format == DataFormat.Float16:
        mantissa_msb = 0x400  # 1 << 10
    elif data_format == DataFormat.Float16_b:
        mantissa_msb = 0x80  # 1 << 7
    elif data_format == DataFormat.Float32:
    ...
    # # Append 1 as MSB to each mantissa (for normalized numbers)
    # if data_format == DataFormat.Float16:
    #     mantissa_msb = 0x400  # 1 << 10
    # elif data_format == DataFormat.Float16_b:
    #     mantissa_msb = 0x80   # 1 << 7
