...
inline void _unified_pack_hw_configure_(UnifiedOperand dst, bool is_32bit_dest, [[maybe_unused]] StochRndType rounding_mode)
{
    if (is_32bit_dest)
    {
        configure_pack<true, false>((uint)dst.dest_format, (uint)dst.l1_format, 0, dst.face_height, dst.num_faces(), dst.partial_face(), dst.narrow_tile());
    }
    else
    {
        configure_pack<false, false>((uint)dst.dest_format, (uint)dst.l1_format, 0, dst.face_height, dst.num_faces(), dst.partial_face(), dst.narrow_tile());
    }
