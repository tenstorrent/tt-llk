    ...
    state_t<int32_t> num_faces_B)
{
    ...
    LLK_SAN_PANIC(state.src_b.output_format != dst_fmt_B, "panic: llk_san: dst_fmt_B doesn't match state.src_b.output_format");
    LLK_SAN_PANIC(state.src_a.face_height != face_height_A, "panic: llk_san: face_height_A doesn't match state.src_a.face_height");
    LLK_SAN_PANIC(state.src_b.face_height != face_height_B, "panic: llk_san: face_height_B doesn't match state.src_b.face_height");
    LLK_SAN_PANIC(state.src_a.num_faces != num_faces_A, "panic: llk_san: num_faces_A doesn't match state.src_a.num_faces");
    LLK_SAN_PANIC(state.src_b.num_faces != num_faces_B, "panic: llk_san: num_faces_B doesn't match state.src_b.num_faces");
}
    ...
    state_t<int32_t> narrow_tile)
{
    ...
    LLK_SAN_PANIC(state.face_height != face_height, "panic: llk_san: face_height doesn't match state.face_height");
    LLK_SAN_PANIC(state.tile_width != tile_width, "panic: llk_san: tile_width doesn't match state.tile_width");
    LLK_SAN_PANIC(state.num_faces != num_faces, "panic: llk_san: num_faces doesn't match state.num_faces");
    LLK_SAN_PANIC(state.partial_face != partial_face, "panic: llk_san: partial_face doesn't match state.partial_face");
    LLK_SAN_PANIC(state.narrow_tile != narrow_tile, "panic: llk_san: narrow_tile doesn't match state.narrow_tile");
}
