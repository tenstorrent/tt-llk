    ...
    state_t<uint32_t> num_faces_B)
{
    ...
    // LLK_SAN_OP_ASSERT(state.dest_width_32, dest_acc_en, "panic: llk_san: dest_acc_en doesn't match state.dest_width_32");

    // LLK_SAN_OP_ASSERT(state.src_a.input_format, src_fmt_A, "panic: llk_san: src_fmt_A doesn't match state.src_a.input_format");
    // LLK_SAN_OP_ASSERT(state.src_b.input_format, src_fmt_B, "panic: llk_san: src_fmt_B doesn't match state.src_b.input_format");
    // LLK_SAN_OP_ASSERT(state.src_a.output_format, dst_fmt_A, "panic: llk_san: dst_fmt_A doesn't match state.src_a.output_format");
    // LLK_SAN_OP_ASSERT(state.src_b.output_format, dst_fmt_B, "panic: llk_san: dst_fmt_B doesn't match state.src_b.output_format");
    // LLK_SAN_OP_ASSERT(state.src_a.face_height, face_height_A, "panic: llk_san: face_height_A doesn't match state.src_a.face_height");
    // LLK_SAN_OP_ASSERT(state.src_b.face_height, face_height_B, "panic: llk_san: face_height_B doesn't match state.src_b.face_height");
    // LLK_SAN_OP_ASSERT(state.src_a.num_faces, num_faces_A, "panic: llk_san: num_faces_A doesn't match state.src_a.num_faces");
    // LLK_SAN_OP_ASSERT(state.src_b.num_faces, num_faces_B, "panic: llk_san: num_faces_B doesn't match state.src_b.num_faces");
}
...
{
    // LLK_SAN_PANIC(!state.is_configured, "panic: llk_san: executing init/execute/uninit before hwconfigure");
    // LLK_SAN_OP_ASSERT(state.src_a.input_format, math_fmt_A, "panic: llk_san: math_fmt_A doesn't match state.src_a.input_format");
    // LLK_SAN_OP_ASSERT(state.src_b.input_format, math_fmt_B, "panic: llk_san: math_fmt_B doesn't match state.src_b.input_format");
}
    ...
    state_t<bool> narrow_tile)
{
    // LLK_SAN_PANIC(!state.is_configured, "panic: llk_san: executing init/execute/uninit before hwconfigure");
    // LLK_SAN_OP_ASSERT(state.dest_width_32, dest_acc_en, "panic: llk_san: dest_acc_en doesn't match state.dest_width_32");
    // LLK_SAN_OP_ASSERT(state.input_format, src_fmt, "panic: llk_san: src_fmt doesn't match state.input_format");
    // LLK_SAN_OP_ASSERT(state.output_format, dst_fmt, "panic: llk_san: dst_fmt doesn't match state.output_format");
    // // sstanisic fixme: LLK_SAN_OP_ASSERT(state.face_height, face_height, "panic: llk_san: face_height doesn't match state.face_height");
    // LLK_SAN_OP_ASSERT(state.tile_width, tile_width, "panic: llk_san: tile_width doesn't match state.tile_width");
    // LLK_SAN_OP_ASSERT(state.num_faces, num_faces, "panic: llk_san: num_faces doesn't match state.num_faces");
    // LLK_SAN_OP_ASSERT(state.partial_face, partial_face, "panic: llk_san: partial_face doesn't match state.partial_face");
    // LLK_SAN_OP_ASSERT(state.narrow_tile, narrow_tile, "panic: llk_san: narrow_tile doesn't match state.narrow_tile");
}
...
{
    LLK_SAN_PANIC(state.operation != op, "operation type doesn't match stored operation");
    ...
    LLK_SAN_ASSERT(std::memcmp(&args_count, ptr, sizeof(args_count)) == 0, "saved vs provided args_count mismatch");
    ptr += sizeof(args_count);

    if constexpr (args_count > 0)
    {
        // LLK_SAN_ASSERT(std::memcmp(args_sizeof.data(), ptr, args_count * sizeof(args_sizeof[0])) == 0, "saved vs provided args_sizeof mismatch");
        ptr += args_count * sizeof(args_sizeof[0]);

        // LLK_SAN_ASSERT(std::memcmp(args_alignof.data(), ptr, args_count * sizeof(args_alignof[0])) == 0, "saved vs provided args_alignof mismatch");
        ptr += args_count * sizeof(args_alignof[0]);
        ...
        size_t i = 0;
        ([&] { LLK_SAN_ASSERT(std::memcmp(ptr + args_offsetof[i++], &args, sizeof(args)) == 0, "saved vs provided args mismatch"); }(), ...);
    }
}
...
{
    LLK_SAN_PANIC(!state.expect_uninit, "operation was not expected to be uninitialized");
...
{
    LLK_SAN_PANIC(current == fsm_state_t::INITIAL && next != fsm_state_t::CONFIGURED, "expected first operation to be configure");

    LLK_SAN_PANIC(current == fsm_state_t::CONFIGURED && next != fsm_state_t::INITIALIZED, "expected init after configure");

    LLK_SAN_PANIC(current == fsm_state_t::INITIALIZED && next != fsm_state_t::EXECUTED, "expected execute after init");
