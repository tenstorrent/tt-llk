...
## Writing template and runtime parameters
TODO
...
![Tensix Core to Chip](images/compilation-graph.svg)
<center><i>Kernel compilation graph</i></center>

## Firmware
This is the C-Runtime (`tmu-crt0.S`), and the `void main()` that every RISCV core in every test executes. Its code is located in `trisc.cpp`, which is a source used to compile `main_unpack.o`, `main_math.o`, `main_pack.o`. These are shared artifacts, and are compiled once, as part of the first queued test. Like any kernel, it has *coverage* and *performance* variants, therefore it is recompiled when these parameters change.
## Functional tests
TODO
## Coverage tests
TODO
...
5. TRISC\[0,1,2] start addresses buffer &rarr; `TestConfig.TRISC_START_ADDRS` in `test_config.py`
